...
namespace IES.Core
{
	...
	public class Cache : ICache
	{
		...
		public Cache(IMemoryCache cache, ILogger<Cache> logger)
		{
			this._cache = cache;
			this._log = logger;
		}
		...
		public void CacheEntryRemoved(object key, object value, EvictionReason reason, object state)
		{
			...
			if (reason != EvictionReason.Removed)
			{
				...
				this._log.LogDebug("CACHE REMOVED : entry key " + keyString + " removed because " + reason.ToString());
			}
		}
		...
		public void Add(string inKey, object inValue, int inSecondsToCacheItems)
		{
			...
			this._log.LogDebug(string.Format("Adding key {0} for value {1} and will cache for {2} seconds", key, inValue.ToString(), inSecondsToCacheItems));
			...
			lock (LOCK)
			{
				this._cache.Set(key, inValue, options);
			}
		}
		...
		public void AddAbsolute(string inKey, object inValue, int inSecondsToCacheItems)
		{
			...
			string key = inKey.ToLower();
			this._log.LogDebug($"Adding key {key} for value {inValue.ToString()} and will cache for {inSecondsToCacheItems} seconds");
		...
		public void Remove(string inKey)
		{
			...
			string key = inKey.ToLower();
			this._log.LogDebug(string.Format("Removing cached item for key {0}", key));
