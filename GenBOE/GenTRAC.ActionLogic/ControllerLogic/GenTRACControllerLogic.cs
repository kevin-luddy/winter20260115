...
namespace GenTRAC.ActionLogic
{
	...
	public class GenTRACControllerLogic
	{
		...
		public FullProposal ValidateChecklist(ChecklistGeneralInformationModelView checklistGeneralInfo, ChecklistProposalPricingDataModelView checklistProposalPricingData, ChecklistProposalPricingReviewDocumentModelView checklistPPRDocumentData, ChecklistProposalAdequacyReviewDocumentModelView checklistPARDocumentData, bool isSubmit, ICollection<ValidationMessage> inValidationErrors)
		{
			...
			if (checklistGeneralInfo.ProposalChecklistID <= 0 && fullProposal.ProposalChecklistData.Any())
			{
			...
			if (checklistProposalPricingData.UseCostThroughCom)
			{
				long.TryParse(checklistProposalPricingData.CostThroughCom?.Replace(",", string.Empty), out long costThroughCom);
				long.TryParse(checklistProposalPricingData.ProfitFeeTotal?.Replace(",", string.Empty), out long profitFeeTotal);
				long.TryParse(checklistGeneralInfo.SubmittedValue?.Replace(",", string.Empty), out long submittedValue);

				if (costThroughCom + profitFeeTotal != submittedValue)
				{
					inValidationErrors.Add(new ValidationMessage(ValidationConstants.ChecklistValidationConstants.COST_THRU_COM_PROFIT_FEE_SUM));
				}
			}
