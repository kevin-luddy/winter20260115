	...
	public abstract class WorkspaceExporter
	{
		...
		public string ExportToExcelFile(string inTemplateFileLocation, BOEExportInputs exportInputs, MetricNameTaskElementMappingDTO metricNameTaskElementMappingDTO, ICollection<PickListDto> contractTypes)
		{
			...
			if (!exportInputs.Workspace.UsingTemplateBOE)
			{
				...
				{
					ExcelUtilities.HideWorksheets(document, new List<string>() { "MOQ by BOE by Task", "MOQ Table Data", "Current Skill Mix", "Common Disclosure Skill Mix" });
				}
			}
			else if (!usingSkillMix)
			{
			...
			toReturn = ExcelExporter.ExportToExcelFile(toReturn, false, sheets,
				this.GetStartRows(exportInputs.Workspace.UsingTemplateBOE, usingSkillMix),
				FullObjectHelper.ShowEquivalentPersonsOption && exportInputs.Workspace.IsUsingEquivalentPerson);
		...
		protected virtual int?[] GetStartRows(bool usingTemplateBoe, bool usingSkillMix)
		{
			if (usingTemplateBoe)
			{
				if (usingSkillMix)
		...
		protected virtual ExcelExportWorksheet GetCurrentSkillMixTableData(BOEExportInputs exportInputs)
		{
			...
			foreach (BoeDTO boe in exportInputs.Boes)
			{
				...
				{
					if (Utilities.ShowSkillMixForTask(exportInputs.Workspace.CreationDate, BOETaskUtility.IsUsingTMRates(exportInputs.FullWorkspace, tmRates, task)))
					{
			...
			foreach (BoeDTO boe in boes)
			{
				...
				foreach (BoeTaskElementDTO task in tasks)
				{
					...
					foreach (ResourceTypeDto resourceType in task.taskElementLabors)
					{
						...
						ResourceDTO brcResource = resourceType.BusinessResourceCodeID.HasValue ? exportInputs.ResourcesUsedInWsBoes.First(x => x.Id == resourceType.BusinessResourceCodeID.Value) : new ResourceDTO();
				...
				foreach (BoeTaskElementDTO task in laborTasks)
				{
					...
					foreach (ResourceTypeDto type in task.taskElementLabors.Where(l => l.StartDate.HasValue && l.EndDate.HasValue))
					{
						...
						while (currentDate <= type.EndDate.Value)
						{
							...
							ResourceDTO brcResource = type.BusinessResourceCodeID.HasValue ? workspaceResources.First(x => x.Id == type.BusinessResourceCodeID.Value) : new ResourceDTO();
		...
		private ExcelExportWorksheet GetLaborResourceTypeDataforBOEResourceCombo(BOEExportInputs exportInputs, ExcelExportWorksheet toReturn, BoeDTO boe, BoeTaskElementDTO task, HashSet<WbsDTO> allWbs, HashSet<ClinDTO> allClins, IReadOnlyCollection<ResourceDTO> workspaceResources, IReadOnlyCollection<CustomFieldDTO> workspace_customFields, ICollection<CustomFieldValueDTO> workspaceCustomFieldValues, IDictionary<int, SpreadCurveModelView> allSpreadCurves, HashSet<PerformingOrgDTO> perfOrgsFromDb, string[] taskFields1, string[] taskFields2)
		{
			foreach (ResourceTypeDto resourceType in task.taskElementLabors)
			{
				...
				ResourceDTO brcResource = resourceType.BusinessResourceCodeID.HasValue ? workspaceResources.First(x => x.Id == resourceType.BusinessResourceCodeID.Value) : new ResourceDTO();
