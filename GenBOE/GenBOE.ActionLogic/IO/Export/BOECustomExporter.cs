	...
	public class BOECustomExporter : WordExporter, IBOECustomExporter
	{
			...
			CustomFieldDTO revCodeCustomField, CustomFieldDTO taskSegregationCustomField)
		{
			...
			BOEExportModelView boeExportModelView = new BOEExportModelView
			{
				...
				DataSource = BOEExportConverter.GetRteOverride(boe.Id, null, boe.DataSource, IES.Common.RteTemplateSource.BoeSources, exportInputs.RTETemplatesOverrides),
				IsMaterial = boe.isMaterial,
				IsMultiClinWbs = boe.IsMultiClinWbs,
				StartDate = boe.StartDate,
				EndDate = boe.EndDate,
				BOEDescription = BOEExportConverter.GetRteOverride(boe.Id, null, boe.Description, IES.Common.RteTemplateSource.BoeDescription, exportInputs.RTETemplatesOverrides),
			...
			ICollection<BoeCustomReportComponent> selectedComponents, BOEExportInputs exportInputs, FullWorkspace ws, ref ChunkCounter counters)
		{
			...
			if (selectedComponents.Contains(BoeCustomReportComponent.TaskMOQEquation))
			{
				...
				laborTaskHeaderDataValueMappings.Add(BOEExporterConstants.FieldName_MOQEquationResult, this.GetMOQTotal(laborTaskElement, exportInputs, ws.TaskElements));
			}
			else
			{
		...
		protected virtual List<LaborRollupByDateNew> GetRollupByYear(ICollection<BoeTaskElementDTO> taskElementCollection, Collection<ResourceDTO> resources = null, RateType? rateTypeFilter = null, bool useGfy = false, bool includeUcot = false)
		{
			...
			else
			{
				...
				allTaskElementResources = taskElementCollection.SelectMany(x => x.taskElementLabors).Where(r => r.ResourceID.HasValue 
					&& (resourceIds.Contains(r.ResourceID.Value) || (includeUcot && r.ResourceID <= Constants.UCOT_RESOURCE_ID))).ToList().DeepClone();
			}
		...
		private decimal GetRollupForMonth(List<ResourceTypeDto> resourceTypeCollection, int year, int month, Collection<ResourceDTO> resources, RateType? rateTypeFilter, bool includeUcot)
		{
			...
			else
			{
				...
				sum =
					(from f in resourceTypeCollection
					 where f.SpreadType == SpreadType.Hours && (resourceIds.Contains(f.ResourceID.Value) || (includeUcot && f.ResourceID.Value <= Constants.UCOT_RESOURCE_ID))
					 from g in f.LaborSpreads
					 where g.LaborSpreadDate.Year == year && g.LaborSpreadDate.Month == month
					 select g.LaborSpreadValue).Sum();
			}
