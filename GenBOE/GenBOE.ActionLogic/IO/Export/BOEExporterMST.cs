...
namespace GenBOE.ActionLogic.IO.Export
{
    ...
    public class BOEExporterMST : BOEExporter
    {
        ...
        protected override void PopulateBoeYearSummaryRollup(SdtElement element, Collection<LaborRollupByDate> laborRollup, String Format, NumberFormatInfo NumberFormatter, String Font, String FontSize, String headerFontSize, string tag, JustificationValues numberAlignment = JustificationValues.Right)
        {
            if (element == null)
            {
            ...
            if (laborRollup.Select(x => x.Total).Sum() > 0)
            {
                ...
                if (tag.Contains("TopTotal")) //if total goes on top of table, add now
                {
                    ...
                    this.PopulateTableCell(tr, label, Font, "24", labelTCP, leftPP, boldRP);
                    ...
                    this.PopulateTableCell(tr, total, Font, "24", totalTCP, leftPP, boldRP, new GridSpan() { Val = 2 });
                    if (NumberFormatter != null)
                    {
                ...
                table.Append(this.CreateRollupHeaderRow(Headers, Font, headerFontSize));
                ...
                foreach (LaborRollupByDate item in laborRollup)
                {
                    ...
                    this.PopulateTableCell(tr2, item.Year.ToString(), Font, FontSize, tcp, leftPP);
                    this.PopulateTableCell(tr2, item.January.ToString(Format, NumberFormatter), Font, FontSize, tcp, rightPP);
                    this.PopulateTableCell(tr2, item.February.ToString(Format, NumberFormatter), Font, FontSize, tcp, rightPP);
                    this.PopulateTableCell(tr2, item.March.ToString(Format, NumberFormatter), Font, FontSize, tcp, rightPP);
                    this.PopulateTableCell(tr2, item.April.ToString(Format, NumberFormatter), Font, FontSize, tcp, rightPP);
                    this.PopulateTableCell(tr2, item.May.ToString(Format, NumberFormatter), Font, FontSize, tcp, rightPP);
                    this.PopulateTableCell(tr2, item.June.ToString(Format, NumberFormatter), Font, FontSize, tcp, rightPP);
                    this.PopulateTableCell(tr2, item.July.ToString(Format, NumberFormatter), Font, FontSize, tcp, rightPP);
                    this.PopulateTableCell(tr2, item.August.ToString(Format, NumberFormatter), Font, FontSize, tcp, rightPP);
                    this.PopulateTableCell(tr2, item.September.ToString(Format, NumberFormatter), Font, FontSize, tcp, rightPP);
                    this.PopulateTableCell(tr2, item.October.ToString(Format, NumberFormatter), Font, FontSize, tcp, rightPP);
                    this.PopulateTableCell(tr2, item.November.ToString(Format, NumberFormatter), Font, FontSize, tcp, rightPP);
                    this.PopulateTableCell(tr2, item.December.ToString(Format, NumberFormatter), Font, FontSize, tcp, rightPP);
                ...
                if (!tag.Contains("TopTotal"))
                {
                    ...
                    this.PopulateTableCell(tr3, "Total:\u00A0", Font, headerFontSize, labelTCP, leftPP, boldRP, new GridSpan() { Val = 1 });
                    ...
                    TableCellProperties totalTCP = new TableCellProperties(new TableCellVerticalAlignment() { Val = TableVerticalAlignmentValues.Center }, new TableCellWidth() { Type = TableWidthUnitValues.Pct });
                    this.PopulateTableCell(tr3, laborRollup.Select(x => x.Total).Sum().ToString(Format, NumberFormatter), Font, FontSize, totalTCP, rightPP, boldRP, new GridSpan() { Val = 2 });
                    if (NumberFormatter != null)
                    {
                        NumberFormatter.CurrencySymbol = String.Empty;
                    }
        ...
        protected override Collection<LaborRollupByDate> GetCostRollup(BOEExportInputs exportInputs, BOEExportModelView boeExportModelView)
        {
            ...
            if (exportInputs != null)
            {
                ...
                {
                    DateRange dateRange = this.GetAllTasksDateRange(LaborElements, ODCElements, TravelElements, MaterialElements, boeExportModelView);
        ...
        private DateRange GetAllTasksDateRange(ICollection<BoeTaskElementDTO> LaborElements, ICollection<OtherDirectCostDTO> ODCElements, ICollection<TravelDTO> TravelElements, ICollection<MaterialDTO> MaterialElements, BOEExportModelView boeExportModelView)
        {
            ...
            odcTravelRange = this.GetODCTravelDateRange(ODCElements, TravelElements);
            laborRange = this.GetTaskDateRange(LaborElements, boeExportModelView);
            materialRange = this.GetMaterialTaskDateRange(MaterialElements);
