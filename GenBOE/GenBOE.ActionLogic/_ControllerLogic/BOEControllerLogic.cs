...
namespace GenBOE.ActionLogic.ControllerLogic
{
	...
	public class BOEControllerLogic : IBOEControllerLogic, IDisposable
	{
		...
		/// </summary>
		/// <param name="ws">Full workspace</param>
		/// <param name="inputStream">The input stream of the imported file</param>
		/// <returns>A collection of BOEs to be imported</returns>
			...
			foreach (ImportedBoe importResult in importResults)
			{
				...
				foreach (BoeImportResult resultType in importResult.ImportTypes)
				{
					results.Add(new ImportedBoe()
					{
						...
						ImportType = (int)resultType,
						ImportTypes = importResult.ImportTypes,
						WBSID = importResult.WBSID,
						WbsString = importResult.WbsString,
						CLINID = importResult.CLINID,
						ClinString = importResult.ClinString,
		...
		public void CompleteImportBOEs(FullWorkspace ws, ICollection<ImportBoeResultsModelView> importResults)
		{
			...
			if (importResults != null && ws != null)
			{
				foreach (ImportBoeResultsModelView boeImportResult in importResults)
				{
					...
					if (boeImportResult.BoeXrefID == null && boeImportResult.WCBID == null)
					{
						boe.WCBID = -1;
					}
					else if (boeImportResult.BoeXrefID != null)
					{
						boe.WCBID = boeImportResult.BoeXrefID;
					}
					else
					{
						boe.WCBID = boeImportResult.WCBID;
					}
					...
					{
						boe.Updateable = UpdateType.Upsert;
						boe.UpdateDate = !(ws.Boes.Any(i => i.Id == boe.Id)) ? DateTime.Now : ws.Boes.First(i => i.Id == boe.Id).UpdateDate;
					}
