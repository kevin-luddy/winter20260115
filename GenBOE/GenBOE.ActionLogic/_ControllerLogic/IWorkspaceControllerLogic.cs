...
namespace GenBOE.ActionLogic.ControllerLogic
{
	...
	public interface IWorkspaceControllerLogic
	{
		...
		/// <summary>
		/// Get the next unique workspace name by checking existing names and shortnames with cascading uniqueness.
		/// If workspace name conflicts, increment suffix. If shortname/URL conflicts, also increment suffix.
		/// </summary>
		...
		Dictionary<string, object> GetNextUniqueWorkspaceName(IEnumerable<string> existingNames, IEnumerable<string> existingShortnames, string baseName);
