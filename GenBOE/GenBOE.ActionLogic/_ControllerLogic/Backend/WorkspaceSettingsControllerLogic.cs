...
namespace GenBOE.ActionLogic._ControllerLogic.Backend
{
	...
	public class WorkspaceSettingsControllerLogic
	{
		...
		/// </summary>
		private IFullWorkspaceRecalculation fullWorkspaceRecalculation { get; set; }
		...
		public WorkspaceSettingsControllerLogic(
			...
			IFullWorkspaceRecalculation fullWorkspaceRecalculation,
		...
		)
		{
			...
			this.fullWorkspaceRecalculation = fullWorkspaceRecalculation;
		...
		/// <summary>
		/// Saves the Workspace Identification
		/// </summary>
		...
		public string SaveWorkspaceIdentification(IFullObjectFactory factory, FullWorkspace ws, IWorkspaceIdentificationModelView workspaceDetails)
		{
			...
			bool ptmTrackingNumberNotRequired = !string.IsNullOrEmpty(ConfigurationUtilities.GetAppSetting("CanCreateWorkspaceWithoutPtmTrackingNumber")) && _securityInformation.IsMemberOfADGroupInAppSettingsList(this._securityInformation.ActiveUserNTID, "CanCreateWorkspaceWithoutPtmTrackingNumber");
			...
			try
			{
				...
				{
					this.workspaceControllerLogic.ChangeTheWorkspaceStateDuringRecalculation(ws, currentUserID, WorkspaceState.Initialization, DateTime.Now);

					// The timeout for the transaction needs to be longer, since the recalculation will save a lot more data..
					timeout = new TimeSpan(0, 10, 0);

					this.workspaceControllerLogic.WsRecalculationStep1(ws, ref tasksToSave, ref workspaceVariablesToSave, ref boesToTransition, originalWsBoes, decimalPrecisionChanged, costDecimalPrecisionChanged, workspaceDetails.CostDecimalPrecision);
				}
				...
				ICollection<MoqTypeSelection> moqTypes = templateBoeUsageChanged ? this.workspaceControllerLogic.GetMoqTypesDataForBoeTemplateSettingChange(ws) : new List<MoqTypeSelection>();
				...
				using (TransactionScope scope = new TransactionScope(TransactionScopeOption.Required, new TransactionOptions { IsolationLevel = IsolationLevel.Snapshot, Timeout = timeout }))
				{
					...
					// Save the workspace
					this.workspaceDTODataLoader.SaveWorkspaceSettings(currentUserID, ws);

					this.workspaceControllerLogic.SaveMoqTypes(moqTypes);
					...
					if (!ws.IsProjectMapWorkspace)
					{
						...
						{
							fullWorkspaceRecalculation.SaveDataEffectedByRecalculation(ws, tasksToSave, workspaceVariablesToSave, boesToTransition);
						}

						// If we recalculated, we need to carry out the actions based on the state transition of Boes
						if (decimalPrecisionChanged)
						{
							fullWorkspaceRecalculation.PerformStateTransitionActionsForBoesEffectedByRecalculation(ws, boesToTransition, originalWsBoes);
						}
