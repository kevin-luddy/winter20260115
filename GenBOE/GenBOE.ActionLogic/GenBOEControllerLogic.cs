...
namespace GenBOE.ActionLogic
{
    ...
    public class GenBOEControllerLogic : IGenBOEControllerLogic
    {
        ...
        /// </summary>
        public virtual int PiwikSiteID 
        { 
            get
            {
                return 13;
        ...
        /// <summary>
        /// Gets the company specific genBoe URL that we want to collect metrics for
        /// </summary>
        public virtual Uri SiteURL
        {
            get
            {
                // we only want to collect metrics from production so we don't use the ServerURL key in the web.config
                return new Uri("https://genboe.isgs.lmco.com");
            }
        }
    }
}
