...
namespace GenBOE.ActionLogic.Validation
{
    ...
    public class ResourceUniqueIDValidator : Validator
    {
        ...
        /// Constructor
        /// </summary>
        public ResourceUniqueIDValidator(IResourceDTODataLoader inResourceLoader)
        {
        ...
        public override Collection<string> validation(object value, Collection<Dictionary<string, string>> inData)
        {
            ...
            if (data != null && data.Keys.Contains("ResourceListID") && !data.Keys.Contains("ResourceID"))
            {
                ...
                bool isUnique = this._ResourceLoader.IsResourceNameUnique(valueToValidate, Convert.ToInt32(resourceListID), null);

                if (!isUnique)
                {
                    response.Add("The Resource ID must be unique");
                }
            }
            else if (data != null && data.Keys.Contains("ResourceListID") && data.Keys.Contains("ResourceID"))
            {
                ...
                bool isUnique = this._ResourceLoader.IsResourceNameUnique(valueToValidate, Convert.ToInt32(resourceListID), Convert.ToInt32(resourceID));
