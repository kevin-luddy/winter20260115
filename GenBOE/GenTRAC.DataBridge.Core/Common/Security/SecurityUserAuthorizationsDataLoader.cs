...
namespace GenTRAC.DataBridge.Core.Common.Security
{
	...
	public class SecurityUserAuthorizationsDataLoader : ISecurityUserAuthorizationsDataLoader
	{
		...
		public SecurityUserAuthorizationsDataLoader(IActiveDirectoryService inActiveDirectoryUtilities,
		...
		public IReadOnlyCollection<SecurityPermissionsResponse> GetPermissionsForUser(UserDTO inUserDTO)
		{
			...
			List<SecurityPermissionsResponse> permissionsToReturn = new() { new SecurityPermissionsResponse(PtmRole.NotSet, null) };
			if (inUserDTO != null)
			{
				log.LogInformation("BEGIN GetPermissionsForUser for user " + inUserDTO.Ntid);
				ICollection<IES.Common.Core.Models.GroupData> groups = activeDirectoryUtilities.GetGroupsForUser(inUserDTO.Ntid);
				...
				log.LogInformation("END GetPermissionsForUser for user " + inUserDTO.Ntid);
			}
