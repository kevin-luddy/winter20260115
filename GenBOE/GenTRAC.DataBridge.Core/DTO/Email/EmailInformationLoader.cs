...
namespace GenTRAC.DataBridge.DTO
{
	...
	public class EmailInformationLoader : IEmailInformationLoader
    {
        ...
        public ICollection<EmailInformationDto> GetAllEmailsToBeSent(int? proposalId)
        {
            ...
            using (IES.Core.StopwatchTimer sw = new IES.Core.StopwatchTimer("EmailInformationLoader.GetAllEmailsToBeSent", this.Log))
            {
                // Get the cutoff Date
                DateTime cutoffDate = IES.Core.Utilities.GetWorkflowCutoffDate();
                ...
                else
                {
                    ...
                    DateTime forecastCutoff = DateTime.Now.AddDays(IES.Core.ConfigurationUtilities.GetAppSetting<int>("ForecastedDaysOut"));
        ...
        {
            using (IES.Core.StopwatchTimer sw = new IES.Core.StopwatchTimer("EmailInformationLoader.UpdateEmailSent", this.Log))
            {
                // get the latest version from DB
                ProposalDto proposal = this.ProposalLoader.GetById(proposalId);
                proposal.Updateable = IES.Core.UpdateType.Upsert;
        ...
        public ICollection<EmailInformationDto> GetDocumentReminderEmailsToBeSent()
        {
            ...
            using (IES.Core.StopwatchTimer sw = new IES.Core.StopwatchTimer("EmailInformationLoader.GetDocumentReminderEmailsToBeSent", this.Log))
            {
