...
namespace GenTRAC.DataBridge.Core.DTO.User
{
	...
	public class UserLoader : DataLoader<UserDTO>, IUserLoader
	{
		...
		/// constructor
		/// </summary>
		/// <param name="adUtils">IActiveDirectoryService</param>
		public UserLoader(IActiveDirectoryService adUtils, ILogger<UserLoader> logger) : this(logger)
		{
		...
		/// constructor
		/// </summary>
		public UserLoader(ILogger<UserLoader> logger) : base(logger)
		{
		}
		...
		public ICollection<UserDTO> GetAll()
		{
			...
			using (StopwatchTimer sw = new("UserLoader.GetAllUsers", Log))
			{
		...
		public ICollection<int> GetAllIds()
		{
			...
			using (StopwatchTimer sw = new("UserLoader.GetAllUserIds", Log))
			{
				using (genTRACEntities dbModel = new())
				{
		...
		{
			outUserId = 0;
			using (StopwatchTimer sw = new("UserLoader.UserExists", Log))
			{
				using (genTRACEntities dbModel = new())
				{
		...
		public override ICollection<UserDTO> GetByIds(ICollection<int> ids)
		{
			...
			using (StopwatchTimer sw = new("UserLoader.GetUserById", Log))
			{
				using (genTRACEntities dbModel = new())
				{
		...
		public ICollection<UserDTO> GetUserDTOsByADGroup(string inADGroup)
		{
			...
			ICollection<UserData> users = adUtils.GetAdGroupUsers(inADGroup);
		...
		public UserDTO GetByNtid(string inNtid)
		{
			...
			using (StopwatchTimer sw = new("UserLoader.GetUserByNtid", Log))
			{
				using (genTRACEntities dbModel = new())
				{
		...
		{
			using (StopwatchTimer sw = new("UserLoader.GetUserIdsByGroupId", Log))
			{
		...
		protected override int? Upsert(UserDTO dtoToUpsert)
		{
			...
			using (StopwatchTimer sw = new("UserLoader.UpsertUser", Log))
			{
				if (dtoToUpsert != null)
				{
					// save
		...
		protected override int? Delete(UserDTO dtoToDelete)
		{
			...
			using (StopwatchTimer sw = new("UserLoader.DeleteUser", Log))
			{
				using (genTRACEntities dbModel = new())
				{
