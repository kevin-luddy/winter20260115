...
namespace IES.ActionLogic.Core.IO.Export
{
	...
	public class RdmProPricerExporter
	{
		...
		internal ICollection<IProPricerExportModelView> GetBurdenRateRows(bool isMissionSolutionsExport, bool isCommercialExport)
		{
			...
			List<BurdenElementModelView> orderedList = BurdenElements.OrderBy(x => x.DisplayOrder1LMX).ToList();
			...
			toReturn.Add(new ProPricerBurdenRateExportRowModelView()
			{
				...
				Date = "Date",
				Rates = BurdenElements.OrderBy(x => x.DisplayOrder1LMX).Select(bp => bp.Name).ToList()
			});
