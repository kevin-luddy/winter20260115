...
namespace GenBOE.DataBridge.DTO
{
	...
	public class BOEFormIBOEDTODataLoader : DataLoader<BOEFormIBOEDTO>, IBOEFormIBOEDTODataLoader
	{
		...
		public ICollection<BOEFormIBOEDTO> GetByTrackingNumber(string ptmTrackingNumber)
		{
			ICollection<BOEFormIBOEDTO> toReturn;
			...
			using (StopwatchTimer sw = new StopwatchTimer(this._log))
			{
				using (GenBoeEntities gbe = new GenBoeEntities())
				{
					toReturn = (from b in gbe.BOEFormIBOEs
								...
								where ptmTrackingNumber == w.TrackingNumber && w.CurrentPTMWorkspace
								orderby b.IBOEFormID
								select new BOEFormIBOEDTO
								{
									Approver = b.Approver,
					...
					{
						if (string.IsNullOrWhiteSpace(iboe.ProposalTitle))
						{
							iboe.ProposalTitle = string.IsNullOrWhiteSpace(iboe.WorkspaceRfpNumber)
								? iboe.WorkspaceProposalTitle
								: $"{iboe.WorkspaceProposalTitle} / {iboe.WorkspaceRfpNumber}";
						}

						iboe.Resources = (from xref in gbe.BOEFormIBOEResourcesXREFs
										  where xref.IBOEFormID == iboe.Id
										  from r in gbe.Resources
										  where r.ResourceID == xref.ResourceID
										  select r.ResourceName).ToList();

						iboe.ClinContractXrefs = (from xref in gbe.BOEFormIBOECLINsXREFs
												  where xref.IBOEFormID == iboe.Id
												  from clin in gbe.CLINs
												  where xref.ClinID == clin.CLINID
												  from ct in gbe.ContractTypeLUs
												  where xref.ContractType == ct.ContractTypeID
