    ...
    public class FullProjectMapWorkspace : FullWorkspace
    {
        ...
        public override IReadOnlyCollection<ResourceDTO> ResourcesUsedInWsBoes
        {
            get
            {
                ...
                {
                    ICollection<int> resourceIds = this.TaskElements.SelectMany(x => x.taskElementLabors).Where(x => x.ResourceID.HasValue).Select(x => x.ResourceID.Value).Distinct().ToList();
