...
namespace IES.ActionLogic.ControllerLogic
{
	...
	public class DocumentControllerLogic : IDocumentControllerLogic
	{
		...
		public ICollection<(string rateCode, string parentSectionNumber)> GetSectionsForRateCodes(ICollection<string> rateCodes, int proposalId)
		{
			if (rateCodes == null)
			{
			...
			foreach (string rateCode in rateCodes)
			{
				...
				{
					throw new ArgumentException("Rate Code found with a length not equal to 6: " + rateCode, nameof(rateCodes));
				}

				RdsbRateDetailModelView rate = rates.FirstOrDefault(r => r.RateCode.StartsWith(rateCode));
