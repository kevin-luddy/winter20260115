			...
			foreach (SectionModelView section in sections)
			{
				if ((!section.IsInternalSection.HasValue || !section.IsInternalSection.Value) && section.ContentType == SectionContentType.Section)
				{
					SectionDetailModelView detail = new SectionDetailModelView
					{
						...
						SectionContainsCasbDisclosureCore = section.SectionContainsCasbDisclosureCore,
						SectionContainsCasbDisclosureService = section.SectionContainsCasbDisclosureService,
						IsDisclosureStatementAdequateCore = section.IsDisclosureStatementAdequateCore.HasValue ? section.IsDisclosureStatementAdequateCore.Value : false,
						IsDisclosureStatementAdequateService = section.IsDisclosureStatementAdequateService.HasValue ? section.IsDisclosureStatementAdequateService.Value : false,
						SectionContainsNonComplianceCore = section.SectionContainsNonComplianceCore,
						SectionContainsNonComplianceService = section.SectionContainsNonComplianceService,
						NonComplianceNotificationCore = section.NonComplianceNotificationCore.HasValue ? section.NonComplianceNotificationCore.Value : false,
						NonComplianceNotificationService = section.NonComplianceNotificationService.HasValue ? section.NonComplianceNotificationService.Value : false,
