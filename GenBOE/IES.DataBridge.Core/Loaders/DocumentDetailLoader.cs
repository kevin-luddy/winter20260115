	...
	public class DocumentDetailLoader : DataLoader<DocumentDetailModelView>, IDocumentDetailLoader
    {
		...
		protected override int? Upsert(DocumentDetailModelView dtoToUpsert)
        {
            ...
            using (StopwatchTimer sw = new(this.Log))
            {
                using (IESEntities context = new())
                {
                    result = context.upsertRDSBDocumentInformation(dtoToUpsert.Id, dtoToUpsert.UpdateDate, dtoToUpsert.ProposalId, dtoToUpsert.DocumentCreatedBy, dtoToUpsert.SelectedRevisionId, dtoToUpsert.StartYear, dtoToUpsert.EndYear, dtoToUpsert.ParentSection, (int?)dtoToUpsert.RevisonSegment).First();
                }
            }
