...
*/
GenConstants.IGNORE_CHANGE_INPUT_LIST_CLASSES = ".filter-textbox, .ui-autocomplete-input, .paging-control input, .ignore-dirty";
...
* @param SessionConfig.toolPrefix The tool prefix.
...
function GenPageManager(SessionConfig) {
    ...
    * @private
    */
    var toolPrefix = SessionConfig && SessionConfig.toolPrefix ? SessionConfig.toolPrefx : "";
    ...
    */
    ...
    /**
    * Sets the tools prefix name
    *@param StringToSet String to set Prefix too.
    */
    this.setToolPrefix = function (StringToSet) {
        toolPrefix = StringToSet;
    }

    /**
    * Gets the tools prefix name
    */
    this.getToolPrefix = function () {
        return toolPrefix;
    }
    ...
    */
    this.clearDirty = function (inFormID) {
        ...
        // TODO investigate possiblly clearing all validation
    };
    ...
    */
    this.CreateUrl = function (params) {
        ...
        var extra = [];
        var postUrl = hostURL + '/' + toolPrefix;
    ...
    */
    this.CreatePostURL = function (workspace, controllerName, actionName, params) {
        var postURL = hostURL + '/' + toolPrefix + workspace + '/' + controllerName + '/' + actionName + '/' + params;
        return postURL;
    };
    ...
    */
    this.CreateNoParamsPostURL = function (workspace) {
        var postURL = hostURL + '/' + toolPrefix + workspace;
        return postURL;
    };
  ...
  */
    ...
    */
    this.CreateSystemAdminWithParmsPostURL = function (controllerName, actionName, params) {
        var postURL = hostURL + '/' + toolPrefix + 'default/' + controllerName + '/' + actionName + '/' + params;
        return postURL;
    };
    ...
    */
    this.CreateSystemAdminPostURL = function (controllerName, actionName) {
        var postURL = hostURL + '/' + toolPrefix + 'default/' + controllerName + '/' + actionName;
        return postURL;
    };
...
*            run.
* @param inConfig.KeepSpecialFieldsEditable
*            A Boolean representing if there are special fields present that
*            need to remain editable for certain user roles even if the
*            proposal is locked.
* @param inConfig.EditableSpecialFieldNames
*            Array of 'name' attributes of input elements that should remain editable
*            even if the proposal is locked. 
...
* @param inConfig.ApplySpellChecks
*           True to apply the spell checker setup
...
function GenWidget(inConfig) {
    ...
    */
    ...
    /**
     * Whether or not the widget has special fields that need to remain readonly for certain user roles even when the proposal is locked.
     */
    this.keepSpecialFieldsEditable = false;
    ...
    */
    this.utils = {
        ...
        applyDeleteableRows: function (listMenu) {
            if (!that.readOnly) {
                $("." + GenConstants.BUTTON_CONTAINER_CLASS, listMenu).prepend('<button class="ies disabled" name="delete-button" type="button">Delete</button>');
    ...
    */
    function initialize() {
        // Store the flag in the widget so that it becomes publically accessible.
        if (inConfig != null && inConfig.KeepSpecialFieldsEditable != null) {
            that.keepSpecialFieldsEditable = inConfig.KeepSpecialFieldsEditable;
        }
    ...
    */
    function applyReadOnly() {
        ...
        if (that.isReadOnly()) {
            ...
            // container
            var widgetFields = that.context.find('input:not(.spellcheck-button), textarea, select');

            // If we have certain special fields that need to remain editable if the user has a certain role,
            //  remove the corresponding input elements from the widgetFields object so that they do not get put into
            //  readonly mode.
            if (that.keepSpecialFieldsEditable) {
                if (inConfig.EditableSpecialFieldNames != null && inConfig.EditableSpecialFieldNames.length > 0) {
                    // Iterate through the special field names that should remain editable.
                    $.each(inConfig.EditableSpecialFieldNames, function(index, fieldName) {
                        if (widgetFields != null && widgetFields.length > 0) {
                            // Update the widgetFields array to remove input elements that have a matching name.
                            widgetFields = $.grep(widgetFields, function (wf) { return (wf.name != null && wf.name !== fieldName); });
                        }
                    });

                    // Convert the array back into a jQuery object (so that the $.each iterator works below).
            ...
            widgetFields.each(function (index) {
                ...
                if (currentField.attr("type") != "hidden" && currentField.attr("type") != "radio"
                    ...
                    if (currentField.get(0).tagName == "SELECT") {
                        replacementFieldText = GenHelper.htmlEncode(currentField.children("option:selected").text());
...
* Show the buttons.
*/
GenWidget.prototype.showButtons = function() {
    $("." + GenConstants.BUTTON_CONTAINER_CLASS, GenWidget.context).removeClass('display-none');
    $(".buttons-left", GenWidget.context).removeClass('display-none');
    $(".buttons-right", GenWidget.context).removeClass('display-none');
    $(".ui-datepicker-trigger", GenWidget.context).removeClass('display-none');
    this.refreshModule();
}
...
GenWidget.prototype.refreshModule = function (includeParentModules) {
    ...
    if (isFixed) {
        ...
        contentBlocks.each(function () {
            ...
            var contentHeight = $(this).children('.module-content-center').height();
            $(this).children('.module-content-left').height(contentHeight);
            $(this).children('.module-content-right').height(contentHeight);
        });
        ...
        success: function (response) {
            ...
            if (response > 0 ) {
                ...
                if (response.hasOwnProperty("ResourceID")) {
                    ...
                    $(output).attr("segmentid", response.segmentid);
    ...
    $.ajax({
        ...
        success: function (data, textStatus, jqXHR) {
            ...
            if (options.performValidation) {
                ...
                // this has been upgraded to claer all forms wven those that have been moved to a dialog.
                ...
                if (!(response.indexOf('{') < 0 || response.indexOf('{') > 2)) {
                    ...
                    if (error.ReturnType == "GenValidationException" || error.ReturnType == "IES.Common.Exceptions.GenValidationException") {
                        ...
                        */
                        ...
                        // this has been upgraded to clear all forms when those that have been moved to a dialog.
                        $(that.forms).each(function () {
                            var form = this;
                            form.clearValidationBox();
                        });
...
*
* This method will create a unqiue event tied to this widget. Calls the
...
* @deprecated (Replaced with GenWidget.on)
*
* This method will create a unqiue delegated (simular to a live event) event
...
/**
* A GenGridWidget is a Widget that contains a grid of data that can be modifed inside the grid itself. it tracks changes to each row of data in the grid.
*
* @constructor
* @augments GenWidget
* @param config
...
* @param inFormConfig.IsClassEnvironment
*            Changes if the classified text is included in the do not include warning
...
* about makign this OBE by switching div.buttons to simply button
*/
...
* String formate ( Non destructive leave original object alone)
* 
* @param inJsonObj
