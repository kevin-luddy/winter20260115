...
namespace GenBOE.Web.Controllers
{
	...
	{
		private readonly Logger log = new Logger(typeof(BOEFormController));
		private readonly IBOEFormControllerLogic boeFormControllerLogic = null;
		private readonly IResourceDTODataLoader resourceDTODataLoader = null;
		private readonly InUseDataLoader inUseDataLoader = null;
		private readonly ContractTypeLoader contractTypeLoader = null;
		...
		public ActionResult ExportBOEForm(string workspace, Collection<int> i, Collection<int> p)
		{
			...
			try
			{
				...
				FullWorkspace ws = this.Factory.CreateFullWorkspace(workspace);
				Stopwatch sw = InitializeAction(log, WebConstants.ACTION_EXPORT_INL_FORMS, SecurityPage.ManageBOEForms, SecurityAuthorization.Read, ws, null);
				...
				FinalizeAction(log, WebConstants.ACTION_EXPORT_INL_FORMS, sw);
			}
			catch (Exception ex)
			{
				log.Error(ex);
		...
		public ViewResult CreateBoeForm(string workspace, BOEFormType boeFormType)
		{
			...
			Stopwatch sw = InitializeAction(log, WebConstants.ACTION_CREATE_BOE_FORM, SecurityPage.ManageBOEForms, SecurityAuthorization.ReadUpdate, this.Factory.CreateFullWorkspace(workspace), null);
			...
			FinalizeAction(log, WebConstants.ACTION_CREATE_BOE_FORM, sw);
		...
		public ViewResult UpdateForm(string workspace, int boeFormId, BOEFormType boeFormType)
		{
			...
			Stopwatch sw = InitializeAction(log, "UpdateForm", SecurityPage.ManageBOEForms, SecurityAuthorization.Read, ws, null);
		...
		public ViewResult DisplayManageBOEForms(string workspace)
		{
			...
			// Initialize Action
			Stopwatch sw = InitializeAction(log, WebConstants.ACTION_DISPLAY_MANAGE_BOE_FORMS, SecurityPage.ManageBOEForms, SecurityAuthorization.Read, ws, null);
			ViewResult toReturn = View(WebConstants.VIEW_MANAGE_BOE_FORMS);
			// Finalize Action
			FinalizeAction(log, WebConstants.ACTION_DISPLAY_MANAGE_BOE_FORMS, sw);
			return toReturn;
		}
		...
		public ViewResult DisplayManageBOEFormsGrid(string workspace)
		{
			...
			// Initialize Action
			Stopwatch sw = InitializeAction(log, WebConstants.ACTION_DISPLAY_MANAGE_BOE_FORMS_GRID, SecurityPage.ManageBOEForms, SecurityAuthorization.Read, ws, null);
			...
			// Action Finalize
			FinalizeAction(log, WebConstants.ACTION_DISPLAY_MANAGE_BOE_FORMS_GRID, sw);
