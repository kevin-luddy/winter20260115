...
namespace GenBOE.Web.Controllers
{
    ...
    public class WorkspaceController : GenBOEController
    {
        ...
        private IWorkOfflineImporter _workOfflineImporter;
        private WorkofflineExporter _WorkofflineExporter = null;
        ...
        public WorkspaceController(ISecurityAccess inSecurityAccess,
            ...
            IWorkOfflineImporter inWorkOfflineImporter,
            WorkofflineExporter inWorkofflineExporter,
            ...
            : base(inSecurityAccess, inCommonDataMapper, inSiteMasterUtilities, inSystemMetrics, factory, inUserDTODataLoader, inPermissionsDTOLoader, inControllerLogic)
        {
            ...
            _workOfflineImporter = inWorkOfflineImporter;
            _WorkofflineExporter = inWorkofflineExporter;
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Performance", "CA1809:AvoidExcessiveLocals"), System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Performance", "CA1809:AvoidExcessiveLocals"), System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Design", "CA1031:DoNotCatchGeneralExceptionTypes"), System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Maintainability", "CA1505:AvoidUnmaintainableCode")]
        virtual public ActionResult ImportWorkspaceHomeBOEs(string workspace)
        {
            Stopwatch sw = InitializeAction(_log, "ImportWorkspaceHomeBOEs", SecurityPage.WorkspaceHome, SecurityAuthorization.Read, Factory.CreateFullWorkspace(workspace), null);

            ViewResult toReturn = null;
            ImportWorkofflineResultsModelView theModelView = new ImportWorkofflineResultsModelView();
            WorkofflineImport importResults = null;
            ...
            if (Request.Files.Count > 0 && Request.Files[0].FileName.Length > 0)
            {
                try
                {
                    if (Request.Files.Count > 0 && Request.Files[0].FileName.Length > 0)
                    {
                        ...
                        importResults = _workOfflineImporter.ImportFromExcelFile(Request.Files[0].InputStream, ws);

                        foreach (WorkofflineImportResult item in importResults.ImportTypes)
                        {
                            theModelView.ImportTypes.Add((int)item);
                        }

                        if (!(importResults.ImportTypes.Contains(WorkofflineImportResult.FileError) || importResults.ImportTypes.Contains(WorkofflineImportResult.WorkspaceIDMissingOrInvalid)))
                        {
                            ICollection<FullBoe> importedBoeObjects = this.Factory.CreateFullBoes(importResults.ImportedBoes.Where(x => x != null).Select(x => x.Id).ToCollection());

                            List<int> boeIdsNotUpdate = new List<int>();
                            List<int> taskIdsNotUpdate = new List<int>();
                            List<int> resourceIdsNotUpdate = new List<int>();
                            List<int> resourceSpreadsNotUpdate = new List<int>();

                            foreach (WorkofflineImportedBoe boe in importResults.ImportedBoes)
                            {
                                FullBoe boeDTO = importedBoeObjects.FirstOrDefault(x => x.Id == boe.Id);

                                FullClin clinDTO = (boe != null && boeDTO != null && boeDTO.CLINID.HasValue) ? boeDTO.Clin : null;
                                FullWbs wbsDTO = (boe != null && boeDTO != null && boeDTO.WBSID.HasValue) ? boeDTO.Wbs : null;
                                ImportWorkofflineBOEResultsModelView boemv = null;

                                //initialize to string.empty
                                string CLINDTONumber = string.Empty;
                                string CLINDTOTitle = string.Empty;
                                string WBSDTONumber = string.Empty;
                                string WBSDTOTitle = string.Empty;

                                //use the imported data if provided
                                if (boe != null && (!string.IsNullOrEmpty(boe.ImportedCLINNumber) || !string.IsNullOrEmpty(boe.ImportedCLINTitle)))
                                {
                                    CLINDTONumber = boe.ImportedCLINNumber;
                                    CLINDTOTitle = boe.ImportedCLINTitle;
                                }
                                //otherwise use the DTO if available
                                else if (clinDTO != null)
                                {
                                    CLINDTONumber = clinDTO.ClinNumber;
                                    CLINDTOTitle = clinDTO.ClinTitle;
                                }

                                if (boe != null && (!string.IsNullOrEmpty(boe.ImportedWBSNumber) || !string.IsNullOrEmpty(boe.ImportedWBSTitle)))
                                {
                                    WBSDTONumber = boe.ImportedWBSNumber;
                                    WBSDTOTitle = boe.ImportedWBSTitle;
                                }
                                else if (wbsDTO != null)
                                {
                                    WBSDTONumber = wbsDTO.WbsNumber;
                                    WBSDTOTitle = wbsDTO.WbsTitle;
                                }
                                ...
                                foreach (BoeImportResult resultType in boe.ImportTypes)
                                {
                                    ...
                                    boemv.BOEID = boeDTO != null ? boeDTO.Id : 0;

                                    if (resultType == BoeImportResult.UpdateBoe)
                                    {
                                        theModelView.BoesToUpdate++;
                                    }
                                    else
                                    {
                                        boeIdsNotUpdate.Add(boe.Id);
                                    }
                                    theModelView.Boes.Add(boemv);
                                }
                                ...
                                {                                    
                                    ...
                                    {
                                        temv = new ImportWorkofflineTaskElementResultsModelView()
                                        {
                                            ...
                                            StartDate = taskElement.StartDate.ToString(),
                                            EndDate = taskElement.EndDate.ToString(),
                                            MOQHoursEquation = taskElement.MOQHoursEquation,
                                            WorkspaceVariableIDs = taskElement.WorkspaceVariableIDs,
                                            MOQTypeID = (int)taskElement.MOQType,
                                            MOQText = taskElement.MOQText,
                                            BOETitle = boe.Title,
                                            BOEWBSNumber = WBSDTONumber,
                                            BOEWBSTitle = WBSDTOTitle,
                                            BOECLINNumber = CLINDTONumber,
                                            BOECLINTitle = CLINDTOTitle,
                                            ImportType = (int)resultType
                                        };

                                        // Get the custom fields
                                        foreach (WorkofflineImportedCustomField customfield in taskElement.ImportedCustomFields)
                                        {
                                            ImportWorkofflineCustomFieldModelView cfmv = new ImportWorkofflineCustomFieldModelView()
                                            {
                                                CustomFieldID = customfield.ImportedCustomFieldID,
                                            ...
                                            temv.CustomFields.Add(cfmv);
                                        }

                                        if (resultType == TaskElementImportResult.UpdateTaskElement || resultType == TaskElementImportResult.CreateTaskElement)
                                        {
                                            theModelView.TaskElementsUpdate++;
                                        }
                                        else
                                        {
                                            taskIdsNotUpdate.Add(temv.BOETaskElementID);
                                        }
                                        if (boemv != null)
                                        {
                                            boemv.TaskElements.Add(temv);
                                        }
                                    }

                                    HashSet<ResourceDTO> resourcesFromDb = new HashSet<ResourceDTO>(this._ResourceLoader.GetByIds(taskElement.ImportedResourceTypes.Where(x => x.ResourceID.HasValue).Select(x => x.ResourceID.Value).Distinct().ToList()));
                                    HashSet<PerformingOrgDTO> performingOrgsFromDb = new HashSet<PerformingOrgDTO>(this.perfOrgLoader.GetByIds(taskElement.ImportedResourceTypes.Where(x => x.PerformingOrgID.HasValue).Select(x => x.PerformingOrgID.Value).Distinct().ToList()));

                                    foreach (WorkofflineImportedResourceType resourceType in taskElement.ImportedResourceTypes)
                                    {
                                        ResourceDTO resource = resourceType.ResourceID.HasValue && resourceType.ResourceID.Value > 0 ? resourcesFromDb.First(x => x.Id == resourceType.ResourceID.Value) : null;
                                        String ElementofCost = resource == null ? String.Empty : resource.ElementOfCost.GetDescription();

                                        PerformingOrgDTO PerformingOrg = performingOrgsFromDb.FirstOrDefault(x => x.Id == resourceType.PerformingOrgID);
                                        String PerfOrg = PerformingOrg == null ? String.Empty : PerformingOrg.PerformingOrgDesc;
                                        ImportWorkofflineResourceTypeResultsModelView rtmv = null;

                                        foreach (LaborTypeImportResult resultType in resourceType.ImportTypes)
                                        {
                                            rtmv = new ImportWorkofflineResourceTypeResultsModelView()
                                            {
                                                Resource = resource == null ? String.Empty : resource.ResourceDesc,
                                                PerformingOrg = PerfOrg,
                                                StartDate = resourceType.StartDate.HasValue ? resourceType.StartDate.Value.ToString("MM/yyyy") : String.Empty,
                                                EndDate = resourceType.EndDate.HasValue ? resourceType.EndDate.Value.ToString("MM/yyyy") : String.Empty,
                                                SpreadCurve = resourceType.SpreadCurveID.GetDescription(),
                                                TaskID = taskElement.BOETaskID,
                                                TaskTitle = taskElement.TaskTitle,
                                                BOETitle = boe.Title,
                                                ...
                                                BOECLINNumber = CLINDTONumber,
                                                BOECLINTitle = CLINDTOTitle,
                                                BOELaborTypeID = resourceType.Id,
                                                ResourceID = resourceType.ResourceID.HasValue ? resourceType.ResourceID.Value : 0,
                                                PerformingOrgID = resourceType.PerformingOrgID.HasValue ? resourceType.PerformingOrgID.Value : 0,
                                                SpreadCurveID = resourceType.SpreadCurveID.HasValue ? (int)resourceType.SpreadCurveID.Value : 0,
                                                PercentSpread = resourceType.PercentSpread.HasValue ? resourceType.PercentSpread.Value : 0,
                                                ValueSpread = resourceType.ValueSpread.HasValue ? resourceType.ValueSpread.Value : 0,
                                                PercentSpreadLocked = resourceType.PercentSpreadLocked,
                                                HourSpreadLocked = resourceType.HourSpreadLocked,
                                                ImportType = (int)resultType
                                            };

                                            // Get the custom fields
                                            foreach (WorkofflineImportedCustomField customfield in resourceType.ImportedCustomFields)
                                            {
                                                ImportWorkofflineCustomFieldModelView cfmv = new ImportWorkofflineCustomFieldModelView()
                                                {
                                                    CustomFieldID = customfield.ImportedCustomFieldID,
                                                ...
                                                rtmv.CustomFields.Add(cfmv);
                                            }

                                            if (resultType == LaborTypeImportResult.AddLaborType || resultType == LaborTypeImportResult.UpdateLaborType)
                                            {
                                                theModelView.ResourceTypeUpdate++;
                                            }
                                            else
                                            {
                                                resourceIdsNotUpdate.Add(rtmv.ResourceID);
                                            }
                                            if (temv != null)
                                            {
                                                temv.ResourceTypes.Add(rtmv);
                                            }
                                        }

                                        // Set the spread collection display parts
                                        rtmv.ResourceSpreads.ElementofCost = ElementofCost;
                                        rtmv.ResourceSpreads.Resource = resource == null ? String.Empty : resource.ResourceDesc;
                                        rtmv.ResourceSpreads.PerformingOrg = PerfOrg;
                                        rtmv.ResourceSpreads.StartDate = resourceType.StartDate.HasValue ? resourceType.StartDate.Value.ToString("MM/yyyy") : String.Empty;
                                        rtmv.ResourceSpreads.EndDate = resourceType.EndDate.HasValue ? resourceType.EndDate.Value.ToString("MM/yyyy") : String.Empty;
                                        rtmv.ResourceSpreads.SpreadCurve = resourceType.SpreadCurveID.GetDescription();
                                        rtmv.ResourceSpreads.TaskID = taskElement.BOETaskID;
                                        rtmv.ResourceSpreads.TaskTitle = taskElement.TaskTitle;
                                        rtmv.ResourceSpreads.BOETitle = boe.Title;
                                        rtmv.ResourceSpreads.BOEWBSNumber = WBSDTONumber;
                                        rtmv.ResourceSpreads.BOEWBSTitle = WBSDTOTitle;
                                        rtmv.ResourceSpreads.BOECLINNumber = CLINDTONumber;
                                        rtmv.ResourceSpreads.BOECLINTitle = CLINDTOTitle;

                                        // move the collection level import types to the MV
                                        foreach (LaborSpreadImportResult item in resourceType.ImportedResourceSpreads.ImportTypes)
                                        {
                                            rtmv.ResourceSpreads.ImportTypes.Add((int)item);
                                        }

                                        foreach (WorkofflineImportedResourceSpread resourceSpread in resourceType.ImportedResourceSpreads)
                                        {
                                            ImportWorkofflineResourceSpreadResultsModelView rsmv = new ImportWorkofflineResourceSpreadResultsModelView()
                                            {
                                                BOELaborSpreadID = resourceSpread.Id,
                                                LaborSpreadDate = resourceSpread.LaborSpreadDate.ToString("MM/yyyy"),
                                                LaborSpreadValue = resourceSpread.LaborSpreadValue
                                            };

                                            foreach (LaborSpreadImportResult resultType in resourceSpread.ImportTypes)
                                            {
                                                rsmv.ImportTypes.Add((int)resultType);
                                            }
                                            rtmv.ResourceSpreads.Add(rsmv);
                                        }

                                        if (rtmv.ResourceSpreads.ImportTypes.Contains((int)LaborSpreadImportResult.UpdateSpread) || rtmv.ResourceSpreads.ImportTypes.Contains((int)LaborSpreadImportResult.CreateSpread))
                                        {
                                            theModelView.ResourceSpreadUpdate++;
                                        }
                                        else
                                        {
                                            resourceSpreadsNotUpdate.Add(rtmv.ResourceID);
                                        }
                                    }
                                }
                            }

                            theModelView.BoesToNotUpdate = (boeIdsNotUpdate.Distinct()).Count();
                            theModelView.TaskElementsNotUpdate = (taskIdsNotUpdate.Distinct()).Count();
                            theModelView.ResourceTypeNotUpdate = (resourceIdsNotUpdate.Distinct()).Count();
                ...
                {
                    theModelView.ImportTypes.Add((int)WorkofflineImportResult.FileError);
                    ViewData["ERRORS_OCCURRED"] = true;
                }
                ...
                catch (DateImportException)
                {
                    theModelView.ImportTypes.Add((int)WorkofflineImportResult.FileError);
                    ViewData["ERRORS_OCCURRED"] = true;
                }
                catch (DataErrorImportException)
                {
                    theModelView.ImportTypes.Add((int)WorkofflineImportResult.FileError);
                    ViewData["ERRORS_OCCURRED"] = true;
                }
                ...
                {
                    /*
                     * Exceptions MUST get communicated back to the user; otherwise the spinner on the page will just keep
                     * spinning with no indication that there was an error.
                     * 
                     */
                    theModelView.ImportTypes.Add((int)WorkofflineImportResult.RuntimeException);
                    ViewData["ERRORS_OCCURRED"] = true;
            ...
            {
                ViewData["SERIALIZED_DATA"] = "{}";
            }

            toReturn = View(WebConstants.VIEW_WORKSPACE_WORKOFFLINE_IMPORT_VERIFICATION, theModelView);
            // Finalize Action
            FinalizeAction(_log, "ImportPreviewWorkspaceBOEs", sw);
        ...
        /// </summary>
        /// <param name="importResults">ModelView containing the import data</param>
        /// <param name="workspace">Workspace to import data into</param>
        /// <returns>JsonResult of success.</returns>
        public JsonResult CompleteImportWorkspaceHome(string workspace, ImportWorkofflineResultsModelView importResults)
        {
            // Initialize Action
            FullWorkspace fullWorkspace = this.Factory.CreateFullWorkspace(workspace, true);

            Stopwatch sw = this.InitializeAction(this._log, WebConstants.ACTION_COMPLETE_WORKOFFLINE_IMPORT_WORKSPACE_HOME, SecurityPage.WorkspaceHome, SecurityAuthorization.Read, fullWorkspace, null);
            bool result;
            ...
            try
            {
                ...
                {
                    result = this._ControllerLogic.CompleteImportWorkspaceFromExcel(importResults, fullWorkspace);
                    scope.Complete();
                }
            }
            catch (ArgumentException)
            {
                //something bad happened, set result to false
                result = false;
            }

            JsonResult toReturn = this.Json(new { Status = result });

            // Finalize Action
            this.FinalizeAction(this._log, WebConstants.ACTION_COMPLETE_WORKOFFLINE_IMPORT_WORKSPACE_HOME, sw);
            return toReturn;
        }

        /// <summary>
        /// This function calls the Export BOE partial view with the workspace ID
        /// </summary>
        ///<param name="workspace">the workspace to use in the system</param>
        /// <returns>partial view</returns>
        public JsonResult GetExportBOEModel(string workspace)
        {
            // Action Initialize
            FullWorkspace ws = this.Factory.CreateFullWorkspace(workspace);
            ...
            // Action Finalize
            FinalizeAction(_log, WebConstants.ACTION_GET_EXPORT_BOE_MODEL, sw);

            return Json(sortedModelViews);
        }
        ...
        /// <summary>
        /// Export all workspace BOEs that meet the following 
        /// Workoffline Export criteria:
        ///     Only BOEs assigned to the current user
        ///     Only BOEs with status of Draft
        ///     Exclude BOEs that have a status of Awaiting Approval or Approved
        ///     Exclude BOEs that are Material BOEs
        /// </summary>
        /// <param name="workspace"></param>
        /// <returns></returns>
        public ActionResult ExportAllBOEsWorkoffline(string workspace)
        {
            // Initialize Action
            FullWorkspace ws = this.Factory.CreateFullWorkspace(workspace);
            Stopwatch sw = InitializeAction(_log, "ExportAllBOEsWorkoffline", SecurityPage.WorkspaceHome, SecurityAuthorization.Read, ws, null);
            ...
            // At this point, the collection contains only BOEs assigned to the current user (Author/Subcontractor Author).
            // (Or all BOEs, if we are in Space Systems mode, and the current user is WorkspaceAdmin.)
            //
            // Get a filtered collection of all BOE IDs that may be exported as follows:
            //      Only BOEs with status of Draft
            //      Exclude BOEs that have a status of Awaiting Approval and Approved
            //      Exclude BOEs that are Material BOEs
            ICollection<int> boeIDs = new Collection<int>();
            foreach (ExportBOEModelView modelView in sortedModelViews)
            {
                if (modelView.BoeID.HasValue && (modelView.State == BOEState.Draft || modelView.State == BOEState.DraftLocked) && !modelView.isMaterial)
                {
                    boeIDs.Add(modelView.BoeID.Value);
                }
            }
            ...
            // Generate output (stream) file name
            string fileNameToDisplayToBrowser = string.Format("{0}_BOE_Export.xlsm", ws.WorkspaceName).Replace(",", string.Empty);

            // Call the export function in the business layer and get back the file contents
            _WorkofflineExporter.ExportToExcelResponse(templateFileName, ws, boeObjects, Response, fileNameToDisplayToBrowser);

            // Finalize Action
            FinalizeAction(_log, "ExportAllBOEsWorkoffline", sw);

            return new EmptyResult();
        }

        public ActionResult ExportBOEsWorkoffline(string workspace, [ModelBinder(typeof(IdListModelBinder))] ICollection<int> id)
        {
            // Initialize Action
            FullWorkspace ws = this.Factory.CreateFullWorkspace(workspace);
            ICollection<FullBoe> boes = this.Factory.CreateFullBoesWithRteData(id);

            Stopwatch sw = InitializeAction(_log, "ExportBOEsWorkoffline", SecurityPage.WorkspaceHome, SecurityAuthorization.Read, ws, null);
            ...
            // Call the export function in the business layer and get back the file contents
            _WorkofflineExporter.ExportToExcelResponse(templateFileName, ws, boes, Response, fileNameToDisplayToBrowser);

            // Finalize Action
            FinalizeAction(_log, "ExportBOEsWorkoffline", sw);
        ...
        /// <summary>
        /// Removes any object that is not marked for create or update from the <see cref="ImportWorkofflineResultsModelView"/>
        /// </summary>
        /// <param name="theModelView">the <see cref="ImportWorkofflineResultsModelView"/> to remove objects not being created or updated</param>
        /// <returns>The filtered <see cref="ImportWorkofflineResultsModelView"/></returns>
        private ImportWorkofflineResultsModelView RemoveInvalidObjectsFromImportModelView(ImportWorkofflineResultsModelView theModelView)
        {
            ImportWorkofflineResultsModelView modelViewCopy = new ImportWorkofflineResultsModelView();

            //if the import was totally hosed, return empty ModelView
            if (ShouldWorkOfflineModelViewBeProcessed(theModelView.ImportTypes))
            {

                foreach (ImportWorkofflineBOEResultsModelView boe in theModelView.Boes)
                {

                    //has the same boe already been processed or the boe should not be process
                    if (ShouldBoeBeProcessedForImportType(boe.ImportType) && !modelViewCopy.Boes.Any(b => b.BOEID == boe.BOEID))
                    {
                        //Deep copy of the Boe
                        ImportWorkofflineBOEResultsModelView boeCopy = CreateValidImportBoeCopy(boe);

                        //Iteate over the task elements 
                        foreach (ImportWorkofflineTaskElementResultsModelView task in boe.TaskElements)
                        {
                            //check if the result type should is valid for processing and that the Task Element ID has not been previously processed
                            if (ShouldTaskElementBeProcessedForImportType(task.ImportType) &&
                                (task.BOETaskElementID == -1 || !boeCopy.TaskElements.Any(t => t.BOETaskElementID == task.BOETaskElementID)))
                            {
                                //Create a depp copy of the Task Element
                                ImportWorkofflineTaskElementResultsModelView taskCopy = CreateValidImportTaskElementCopy(task);

                                //Itereate over all the Resource Types in the task
                                foreach (ImportWorkofflineResourceTypeResultsModelView resource in task.ResourceTypes)
                                {
                                    //check if valid Result Type and if the resource is either new or the ID has not already been processed
                                    if (ShouldLaborTypeBeProcessedForImportType(resource.ImportType) &&
                                        resource.BOELaborTypeID < 0 || !taskCopy.ResourceTypes.Any(r => r.BOELaborTypeID == resource.BOELaborTypeID))
                                    {
                                        //Create a depp copy of the Resource Labor Type
                                        ImportWorkofflineResourceTypeResultsModelView laborCopy = CreateValidImportResourceCopy(resource);
                                        //add resource to task element
                                        taskCopy.ResourceTypes.Add(laborCopy);
                                    } //end Resource/Labor type valid
                                }
                                //add task element wtih processed resources to boe
                                boeCopy.TaskElements.Add(taskCopy);
                            } // end Task element valid
                        }
                        //add boe with processed task element to model view
                        modelViewCopy.Boes.Add(boeCopy);
                    } //end BOE valid
                }
            } // end Model View valid

            return modelViewCopy;
        }

        private bool ShouldWorkOfflineModelViewBeProcessed(Collection<int> importTypes)
        {
            if (importTypes.Contains((int)WorkofflineImportResult.WorkspaceIDMissingOrInvalid) ||
                importTypes.Contains((int)WorkofflineImportResult.FileError))
            {
                return false;
            }
            else
        ...
        private bool ShouldBoeBeProcessedForImportType(int boeImportType)
        {
            BoeImportResult boeResult = (BoeImportResult)boeImportType;

            if (boeResult == BoeImportResult.UpdateBoe ||
                    (boeResult != BoeImportResult.BoeIDMissingInvalid && boeResult != BoeImportResult.DuplicateBoeID && boeResult != BoeImportResult.BoeNotInWorkspace))
            {
                return true;
            }
            else
        ...
        /// <summary>
        /// Modifies the data in the boe parameter to remove data marked as bad during validation process
        /// </summary>
        /// <param name="boe"></param>
        private ImportWorkofflineBOEResultsModelView CreateValidImportBoeCopy(ImportWorkofflineBOEResultsModelView boe)
        {
            ...
            {
                boeCopy.CLINID = boe.CLINID;
                boeCopy.CLINNumber = boe.CLINNumber;
                boeCopy.CLINTitle = boe.CLINTitle;
                boeCopy.Description = boe.Description;
                boeCopy.SourcesOfData = boe.SourcesOfData;
                boeCopy.Title = boe.Title;
                boeCopy.WBSID = boe.WBSID;
                boeCopy.WBSNumber = boe.WBSNumber;
                boeCopy.WBSTitle = boe.WBSTitle;
        ...
        private bool ShouldTaskElementBeProcessedForImportType(int taskElementImportType)
        {
            TaskElementImportResult taskResult = (TaskElementImportResult)taskElementImportType;

            if (taskResult == TaskElementImportResult.UpdateTaskElement || taskResult == TaskElementImportResult.CreateTaskElement ||
                                (taskResult != TaskElementImportResult.TaskElementIDMissingOrInvalid &&
                                taskResult != TaskElementImportResult.StartDateMissingInvalid &&
                                taskResult != TaskElementImportResult.EndDateMissingInvalid))
            {
                return true;
            }
            else
        ...
        private ImportWorkofflineTaskElementResultsModelView CreateValidImportTaskElementCopy(ImportWorkofflineTaskElementResultsModelView task)
        {
            ImportWorkofflineTaskElementResultsModelView taskCopy = new ImportWorkofflineTaskElementResultsModelView();
            taskCopy.ImportType = task.ImportType;
            taskCopy.BOETaskElementID = task.BOETaskElementID;

            if ((TaskElementImportResult)task.ImportType == TaskElementImportResult.CreateTaskElement ||
                (TaskElementImportResult)task.ImportType == TaskElementImportResult.UpdateTaskElement)
            {
                taskCopy.BOECLINNumber = task.BOECLINNumber;
                taskCopy.BOECLINTitle = task.BOECLINTitle;
                taskCopy.BOETitle = task.BOETitle;
                taskCopy.BOEWBSNumber = task.BOEWBSNumber;
                taskCopy.BOEWBSTitle = task.BOEWBSTitle;
                taskCopy.Description = task.Description;
                taskCopy.EndDate = task.EndDate;
                taskCopy.MOQHoursEquation = task.MOQHoursEquation;
                taskCopy.MOQText = task.MOQText;
                taskCopy.MOQTypeID = task.MOQTypeID;
                taskCopy.StartDate = task.StartDate;
                taskCopy.TaskID = task.TaskID;
                taskCopy.TaskTitle = task.TaskTitle;
                taskCopy.WorkspaceVariableIDs = task.WorkspaceVariableIDs;

                //trim custom fields that were not set
                taskCopy.CustomFields = task.CustomFields.Where(w => w.CustomFieldID != 0 && w.CustomFieldValueID != 0).ToCollection();
            }

            return taskCopy;
        }

        private bool ShouldLaborTypeBeProcessedForImportType(int laborImportType)
        {
            LaborTypeImportResult laborResult = (LaborTypeImportResult)laborImportType;

            if (laborResult == LaborTypeImportResult.AddLaborType || laborResult == LaborTypeImportResult.UpdateLaborType ||
                (laborResult != LaborTypeImportResult.ResourceTypeIDMissingOrInvalid && laborResult != LaborTypeImportResult.ResourceMissingInvalid &&
                laborResult != LaborTypeImportResult.NonUniqueResourceTypeID && laborResult != LaborTypeImportResult.StartDateMissingInvalid &&
                laborResult != LaborTypeImportResult.EndDateMissingInvalid))
            {
                return true;
            }
            else
        ...
        private ImportWorkofflineResourceTypeResultsModelView CreateValidImportResourceCopy(ImportWorkofflineResourceTypeResultsModelView labor)
        {
            ImportWorkofflineResourceTypeResultsModelView laborCopy = new ImportWorkofflineResourceTypeResultsModelView();
            laborCopy.ImportType = labor.ImportType;

            if ((LaborTypeImportResult)labor.ImportType == LaborTypeImportResult.AddLaborType || (LaborTypeImportResult)labor.ImportType == LaborTypeImportResult.UpdateLaborType)
            {
                laborCopy.BOECLINNumber = labor.BOECLINNumber;
                laborCopy.BOECLINTitle = labor.BOECLINTitle;
                laborCopy.BOELaborTypeID = labor.BOELaborTypeID;
                laborCopy.BOETitle = labor.BOETitle;
                laborCopy.BOEWBSNumber = labor.BOEWBSNumber;
                laborCopy.BOEWBSTitle = labor.BOEWBSTitle;
                laborCopy.EndDate = labor.EndDate;
                laborCopy.PercentSpread = labor.PercentSpread;
                laborCopy.PerformingOrg = labor.PerformingOrg;
                laborCopy.PerformingOrgID = labor.PerformingOrgID;
                laborCopy.Resource = labor.Resource;
                laborCopy.ResourceID = labor.ResourceID;
                laborCopy.SpreadCurve = labor.SpreadCurve;
                laborCopy.SpreadCurveID = labor.SpreadCurveID;
                laborCopy.StartDate = labor.StartDate;
                laborCopy.TaskID = labor.TaskID;
                laborCopy.TaskTitle = labor.TaskTitle;
                laborCopy.ValueSpread = labor.ValueSpread;
                laborCopy.HourSpreadLocked = labor.HourSpreadLocked;
                laborCopy.PercentSpreadLocked = labor.PercentSpreadLocked;
                laborCopy.WbsID = labor.WbsID;
                laborCopy.ClinID = labor.ClinID;
                //Resources and Resource spreads should be processed together here
                //Only process Spreads for Discrete curves, all other spreads are calculated in business logic.
                if (labor.ResourceSpreads != null &&
                    ShouldLaborSpreadTypeBeProcessedForImportType(labor.ResourceSpreads.ImportTypes) &&
                    ((SpreadCurves)labor.SpreadCurveID == SpreadCurves.DiscreteCost || (SpreadCurves)labor.SpreadCurveID == SpreadCurves.DiscreteHours))
                {
                    laborCopy.ResourceSpreads = CreateValidImportResourceSpreadCopy(labor.ResourceSpreads);

                }

                laborCopy.CustomFields = labor.CustomFields.Where(w => w.CustomFieldValueID != 0).ToCollection();
            }

            return laborCopy;
        }

        private bool ShouldLaborSpreadTypeBeProcessedForImportType(Collection<int> laborSpreadImportTypes)
        {
            //This is the only ImportType value that should cause the Spread to be ignored/skipped
            if (laborSpreadImportTypes.Contains((int)LaborSpreadImportResult.SpreadDateOutsideOfLaborTypeDateRange))
            {
                return false;
            }
            else
        ...
        private ImportWorkofflineResourceSpreadResultsModelViewCollection CreateValidImportResourceSpreadCopy(ImportWorkofflineResourceSpreadResultsModelViewCollection laborSpread)
        {
            ImportWorkofflineResourceSpreadResultsModelViewCollection spreadCopy = new ImportWorkofflineResourceSpreadResultsModelViewCollection();

            //AddRange() would have been nice here (like on List<T> instead of Collection<T>)
            foreach (int it in laborSpread.ImportTypes)
            {
                spreadCopy.ImportTypes.Add(it);
            }

            //only deep copy data elements if the SpreadCollection is being updated
            if (laborSpread.ImportTypes.Contains((int)LaborSpreadImportResult.UpdateSpread) || laborSpread.ImportTypes.Contains((int)LaborSpreadImportResult.CreateSpread))
            {
                //now process the individual spread values
                foreach (ImportWorkofflineResourceSpreadResultsModelView s in laborSpread)
                {
                    ImportWorkofflineResourceSpreadResultsModelView newSpreadElement = new ImportWorkofflineResourceSpreadResultsModelView();
                    foreach (int it in s.ImportTypes)
                    {
                        newSpreadElement.ImportTypes.Add(it);
                    }

                    //only copy if the data is being updated
                    if (s.ImportTypes.Contains((int)LaborSpreadImportResult.CreateSpread) || s.ImportTypes.Contains((int)LaborSpreadImportResult.UpdateSpread))
                    {
                        newSpreadElement.BOELaborSpreadID = s.BOELaborSpreadID;
                        newSpreadElement.LaborSpreadDate = s.LaborSpreadDate;
                        newSpreadElement.LaborSpreadValue = s.LaborSpreadValue;
                    }
                    spreadCopy.Add(newSpreadElement);
                }

                //once the spread elements have been copied and added, copy the rest of the data elements
                spreadCopy.BOECLINNumber = laborSpread.BOECLINNumber;
                spreadCopy.BOECLINTitle = laborSpread.BOECLINTitle;
                spreadCopy.BOETitle = laborSpread.BOETitle;
                spreadCopy.BOEWBSNumber = laborSpread.BOEWBSNumber;
                spreadCopy.BOEWBSTitle = laborSpread.BOEWBSTitle;
                spreadCopy.ElementofCost = laborSpread.ElementofCost;
                spreadCopy.EndDate = laborSpread.EndDate;
                spreadCopy.PerformingOrg = laborSpread.PerformingOrg;
                spreadCopy.Resource = laborSpread.Resource;
                spreadCopy.SpreadCurve = laborSpread.SpreadCurve;
                spreadCopy.StartDate = laborSpread.StartDate;
                spreadCopy.TaskID = laborSpread.TaskID;
                spreadCopy.TaskTitle = laborSpread.TaskTitle;
            }
