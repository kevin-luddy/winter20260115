...
<div id="MOQEquationField" class="bootstrap" data-ng-controller="MoqEquationController" data-ng-init="init()">
    ...
    <div data-ng-if="model.UsingTemplateBOE" class="form-row moqContainerClass">
        <div data-ng-if="!model.IsSkillMixEnabled || model.SelectedMoqTypes.length === 0" class="form-label">
            <span>Add New MOQ Type</span>
        </div>
        <div class="form-element">
            <select data-ng-model="model.selectedMOQType" data-ng-if="!ActualReadOnly() && (!model.IsSkillMixEnabled || model.SelectedMoqTypes.length === 0)" data-ng-options="moqType.SelectedMOQTypeText for moqType in model.MOQTypes | moqTypesFilter:model.SelectedMoqTypes" class="moqTypes"></select>
            <button data-ng-if="!ActualReadOnly() && (!model.IsSkillMixEnabled || model.SelectedMoqTypes.length === 0)" data-ng-click="AddMoqType()" data-ng-disabled="!model.selectedMOQType" class="moqTypesButton ies-action" type="button">Add MOQ Type</button>
            <button data-ng-if="!ActualReadOnly() && (!model.IsSkillMixEnabled || model.SelectedMoqTypes.length > 1)" data-ng-disabled="model.SelectedMoqTypes.length <= 1" data-ng-click="displayReOrderMoqTypesDialog()" class="moqTypesButton ies-blue" type="button">Sort MOQ Types</button>
