...
namespace RDSB.Backend.Common
{
	...
	internal static class AuthorizationDictionarySetup
	{
		...
		{
			controller.Add(action.ToLower(CultureInfo.CurrentCulture), authorization);
		}
		...
		private static void AddAdminControllerActions(Dictionary<string, Dictionary<string, SecurityAuthorization>> result)
		{
			...
			result.Add(IES.Common.Core.Constants.IESWebConstants.CONTROLLER_ADMIN.ToLower(CultureInfo.CurrentCulture), adminControllerActions);
		}
