...
namespace GenBOE.DataBridge.Core.Common
{
	...
	public class CommonDataMapper : ICommonDataMapper
	{
		...
		{
			return isProjectMapWorkspace ? GetSikorskyLegacyResources().ToDictionary(x => x.LegacyID) : null;
		}
		...
		{
			return GetSikorskyLegacyResources().ToDictionary(x => x.LegacyResourceID);
		}
		...
		{
			return GetElementOfCostTypes().ToDictionary(x => x.ElementOfCostId);
		}
		#endregion
		...
		{
			return getProposalStatusTypes().ToDictionary(x => x.ProposalStateID);
		}
		#endregion
		...
		{
			return getBOEStates().Where(x => x.BOEStateID == (int)inEnumToGetName).Select(x => x.BOEState).FirstOrDefault();
		}
		...
		{
			return getBOEStates().ToDictionary(x => x.BOEStateID);
		}
		#endregion
		...
		{
			return getMOQType().Where(x => x.MOQTypeID == (int)inEnumToGetName).Select(x => x.MOQTypeName).FirstOrDefault();
		}
		...
		{
			return getMOQType().ToDictionary(x => x.MOQTypeID);
		}
		#endregion
		...
		{
			return getRoles().ToDictionary(x => x.RoleID);
		}
		#endregion
		...
		{
			return getSpreadCurve().ToDictionary(x => (int)x.SpreadCurveID);
		}
		...
		{
			return getProjectMapSpreadCurve().ToDictionary(x => (int)x.SpreadCurveID);
		}
		#endregion
		...
		{
			return getWorkspaceStatesDictionary()[(int)inStateToGetName].WorkspaceState;
		}
		...
		{
			return getWorkspaceStates().ToDictionary(x => x.WorkspaceStateID);
		}
		#endregion
		...
		{
			return getFieldTypes().ToDictionary(x => x.FieldTypeID);
		}
		#endregion
		...
		{
			GetProPricerFieldsDelegate getProPricerFieldsDelegate = _CommonDataLoader.GetProPricerFields;
			object toReturn;
			if (isProjectMapType)
			{
				toReturn = _CacheDataLoader.GetData(getProPricerFieldsDelegate, new[] { (object)isProjectMapType }, CacheConstants.PROJMAP_PROPRICER_FIELDS, false);
			}
			else
			{
				toReturn = _CacheDataLoader.GetData(getProPricerFieldsDelegate, new[] { (object)isProjectMapType }, CacheConstants.PROPRICER_FIELDS, false);
			}
		...
		public virtual IDictionary<int, EnumTypeModelView> GetProPricerFieldsDictionary(bool isProjectMapType, bool enableTaskAuthor)
		{
			ICollection<EnumTypeModelView> proPricerFields = GetProPricerFields(isProjectMapType);

			return proPricerFields
				// Filter Task Author fields after retrieval since the values are cached at a system level and the feature can be enabled/disabled at a Workspace level
				.Where(x => enableTaskAuthor || (x.EnumTypeID != (int)ProPricerField_Task.TaskAuthor && x.EnumTypeID != (int)ProPricerField_Resources.TaskAuthor))
				.ToDictionary(x => x.EnumTypeID);
		}
		#endregion
		...
		{
			return GetRateTypes().ToDictionary(x => x.RateTypeID);
		}
		#endregion
		...
		{
			return getOdcSpreadCurve().ToDictionary(x => (int)x.SpreadCurveID);
		}
		#endregion
		...
		{
			GetEmailDelegate emailDelegate = _CommonDataLoader.GetEmails;
			object toReturn = _CacheDataLoader.GetData(emailDelegate, null, CacheConstants.EMAILS, false);
		...
		public virtual void SaveSystemEmails(ICollection<EmailModelDomain> emails)
		{
			...
			{
				_CommonDataLoader.SaveSystemEmail(email);
			}

			// Remove System email cache
			_CacheDataLoader.Remove(CacheConstants.EMAILS);
		}
		...
		{
			return GetMSTTravelModes().ToDictionary(x => x.EnumTypeID);
		}
		#endregion
	}
}
