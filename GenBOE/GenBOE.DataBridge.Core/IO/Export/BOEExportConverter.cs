...
namespace GenBOE.DataBridge.Core.IO.Export
{
	...
	public class BOEExportConverter : BOEExportUtilities
	{
		...
		public BOEExportConverter(ILogger<BOEExportConverter> logger, ICommonDataMapper commonDataMapper, IVariableSelectBOEtoSumCalculation variableSelectBOEtoSumCalculation,
		...
		public ICollection<BOEExportModelView> ConvertBoeDTOsToExportMVs(BOEExportInputs exportInputs)
		{
			...
			{
				BOEExportModelView modelView = ConvertBoeDTOToExportMV(boe, exportInputs, writelogstatements,
			...
			CustomFieldDTO govtLaborCategoryCustomField, CustomFieldDTO keyPersonnelCustomField)
		{
			...
			{
				Logger.LogInformation("Exporting - BOEExporter - ConvertBoeDTOToExportMV - begin");
				Logger.LogInformation("Exporting - BOEExporter - ConvertBoeDTOToExportMV - get DTO's begin");
			}
			...
			if (writelogstatements)
			{
				Logger.LogInformation("Exporting - BOEExporter - ConvertBoeDTOToExportMV - get DTO's end");
			}
			...
			boeExportModelView.DataSource = BOEExportConverter.GetRteOverride(boe.Id, null, boe.DataSource, IES.Common.Core.Enums.RteTemplateSource.BoeSources, exportInputs.RTETemplatesOverrides);
            ...
            */
			...
			if (writelogstatements)
			{
				Logger.LogInformation("Exporting - BOEExporter - ConvertBoeDTOToExportMV - get segregation info begin");
			}
			...
			if (writelogstatements)
			{
				Logger.LogInformation("Exporting - BOEExporter - ConvertBoeDTOToExportMV - get segregation info end");
			}
			...
			GetCompanySpecificBOECustomFields(workspaceCustomFields, workspaceCustomFieldValues, boe, boeExportModelView);
			...
			boeExportModelView.BOEDescription = BOEExportConverter.GetRteOverride(boe.Id, null, boe.Description, IES.Common.Core.Enums.RteTemplateSource.BoeDescription, exportInputs.RTETemplatesOverrides);
			boeExportModelView.ExportFormat = exportFormatDTO.ExportFormat;

			Collection<BOEExportTaskElement> boeExportTaskElements = PopulateLaborAndMissionTasks(boe,
			...
			PopulateOdcTasks(boe, exportInputs, writelogstatements, boeExportTaskElements);
			PopulateTravelTasks(boe, exportInputs, writelogstatements, boeExportTaskElements);
			PopulateMaterialTasks(boe, exportInputs, writelogstatements, boeExportTaskElements);

			boeExportModelView.TaskElements = boeExportTaskElements;

			if (writelogstatements)
			{
				Logger.LogInformation("Exporting - BOEExporter - ConvertBoeDTOToExportMV - end");
			}
			...
			CustomFieldDTO keyPersonnelCustomField, string boeNumber)
		{
			if (writelogstatements)
			{
				Logger.LogInformation("Exporting - BOEExporter - ConvertBoeDTOToExportMV - labor tasks begin");
			}

			Collection<BOEExportTaskElement> boeExportTaskElements = new Collection<BOEExportTaskElement>();
			IDictionary<int, ElementOfCostTypeModelView> allElementOfCostTypes = commonDataMapper.GetElementOfCostTypesDictionary();
			IDictionary<int, SpreadCurveModelView> allSpreadCurves = exportInputs.Workspace.IsProjectMapWorkspace ? commonDataMapper.getProjectMapSpreadCurveDictionary() : commonDataMapper.getSpreadCurveDictionary();
			IDictionary<int, SikorskyLegacyResourceDTO> allLegacyResources = commonDataMapper.GetSikorskyLegacyResourcesDictionary(exportInputs.Workspace.IsProjectMapWorkspace);
			...
			foreach (BoeTaskElementDTO boeTaskElement in taskElements)
			{
				...
				if (BOETaskUtility.ShowSkillMixForTask(exportInputs.Workspace.CreationDate, boeTaskElement.HasTMRates, boeExportTaskElement.MOQTypes))
				{
				...
				boeExportTaskElement.MOQVariableModelViews = GetSumOfBoeMOQVariableModelView(boeExportTaskElement, exportInputs);
				...
				foreach (ResourceTypeDto laborType in boeTaskElement.taskElementLabors)
				{
					...
					if (laborType.SpreadType == SpreadType.Hours)
					{
						...
						boeExportLabor.ExportFields[BOEExporter.FieldName_LaborTypeHours] = boeExportLabor.Hours.HasValue ? CommonUtilities.FormatStringWithPrecision(boeExportLabor.Hours.Value, WorkspaceDecimalPrecision) : string.Empty;
					}
					else if (laborType.SpreadType == SpreadType.Cost)
					{
						boeExportLabor.Cost = (decimal)laborType.ValueSpread;
						boeExportLabor.ExportFields[BOEExporter.FieldName_LaborTypeCost] = boeExportLabor.Cost.HasValue ? boeExportLabor.Cost.Value.ToString("C0", CurrencyFormatter) : string.Empty;
					}
					...
					if (laborType.ResourceID.HasValue)
					{
						...
						{
							boeExportLabor.ExportFields[BOEExporter.FieldName_ResourceName] = CommonUtilities.FormatResourceNames(resource.ResourceName, commonDataMapper.GetSikorskyLegacyResourceID(laborType.LegacyID, allLegacyResources), laborType is SubResourceTypeDto);
						}
						else
						{
						...
						{
							boeExportLabor.ExportFields[BOEExporter.FieldName_LaborTypeMatSubIwtaCost] = boeExportLabor.Cost.HasValue ? boeExportLabor.Cost.Value.ToString("C0", CurrencyFormatter) : "$0";
						}
						else if (resource.ElementOfCost == ElementOfCostType.Travel || resource.ElementOfCost == ElementOfCostType.ODC)
						{
							boeExportLabor.ExportFields[BOEExporter.FieldName_LaborTypeOtherCost] = boeExportLabor.Cost.HasValue ? boeExportLabor.Cost.Value.ToString("C0", CurrencyFormatter) : "$0";
						}
				...
				boeExportTaskElement.ExportFields[BOEExporter.FieldName_TaskHoursTotal] = CommonUtilities.FormatStringWithPrecision(boeExportTaskElement.taskElementLabors.Where(l => l.Hours.HasValue).Sum(l => l.Hours.Value), WorkspaceDecimalPrecision);
				...
				boeExportTaskElement.ExportFields[BOEExporter.FieldName_TaskCostTotal] = boeExportTaskElement.taskElementLabors
					...
					.Sum(l => l.Cost.Value).ToString("C0", CurrencyFormatter);
				...
				GetCompanySpecificTaskCustomFields(workspaceCustomFields, workspaceCustomFieldValues, boeTaskElement, boeExportTaskElement);
			...
			if (writelogstatements)
			{
				Logger.LogInformation("Exporting - BOEExporter - ConvertBoeDTOToExportMV - labor tasks end");
			}
		...
		private void PopulateOdcTasks(BoeDTO boe, BOEExportInputs exportInputs, bool writelogstatements, Collection<BOEExportTaskElement> boeExportTaskElements)
		{
			if (writelogstatements)
			{
				Logger.LogInformation("Exporting - BOEExporter - ConvertBoeDTOToExportMV - odc tasks begin");
			}
			...
			foreach (OtherDirectCostDTO odcElement in odcElements)
			{
				...
				foreach (var odcType in aggregatedOdcTypes)
				{
					...
					boeExportLabor.ExportFields[BOEExporter.FieldName_LaborTypeCost] = odcType.Cost.ToString("C0", CurrencyFormatter);
				...
				boeExportTaskElement.ExportFields[BOEExporter.FieldName_TaskCostTotal] = boeExportTaskElement.taskElementLabors.Where(l => l.Cost.HasValue).Sum(l => l.Cost.Value).ToString("C0", CurrencyFormatter);
			...
			if (writelogstatements)
			{
				Logger.LogInformation("Exporting - BOEExporter - ConvertBoeDTOToExportMV - odc tasks end");
			}
		}
		...
		/// <param name="writelogstatements">if set to <c>true</c> [writelogstatements].</param>
		/// <param name="boeExportTaskElements">The boe export task elements.</param>
		/// <exception cref="ArgumentNullException">workspace  or boeExportTaskElements</exception>
		protected virtual void PopulateTravelTasks(BoeDTO boe, BOEExportInputs exportInputs, bool writelogstatements, Collection<BOEExportTaskElement> boeExportTaskElements)
		{
			...
			if (writelogstatements)
			{
				Logger.LogInformation("Exporting - BOEExporter - ConvertBoeDTOToExportMV - travel tasks begin");
			}
			...
			foreach (TravelDTO travelElement in travelTasks)
			{
				...
				foreach (TravelTripType travelTrip in travelElement.TravelTrips)
				{
					...
					boeExportLabor.Cost = this.travelTripCostCalculation.CalculateTravelCost(travelTrip, exportInputs.Workspace, exportInputs.WorkspaceHistory, systemTrip, miscRate, perDiem, exportInputs.EscalationRates).CostTotal;
					boeExportLabor.ExportFields[BOEExporter.FieldName_LaborTypeCost] = boeExportLabor.Cost.Value.ToString("C0", CurrencyFormatter);
			...
			if (writelogstatements)
			{
				Logger.LogInformation("Exporting - BOEExporter - ConvertBoeDTOToExportMV - travel tasks end");
			}
		}
		...
		private void PopulateMaterialTasks(BoeDTO boe, BOEExportInputs exportInputs, bool writelogstatements, Collection<BOEExportTaskElement> boeExportTaskElements)
		{
			if (writelogstatements)
			{
				Logger.LogInformation("Exporting - BOEExporter - ConvertBoeDTOToExportMV - materials tasks begin");
			}
			...
			if (writelogstatements)
			{
				Logger.LogInformation("Exporting - BOEExporter - ConvertBoeDTOToExportMV - material tasks end");
			}
		}
				...
				if (item.ValueType == VarValueType.SumOfBOEs)
				{
					...
					foreach (SelectBOEsToSum item2 in item.SelectedBOEsToSum)
					{
						if (item2.BoeID.HasValue && item2.BoeID > 0)
						{
							...
							ovboemv.Total = variableSelectBOEtoSumCalculation.GetTotalBasedOnBoeID(item2.BoeID.Value, item.SumVariableResourceTypeIDs.ToCollection(), data);
					...
					foreach (SelectBOEsToSum item2 in item.SelectedBOEsToSum)
					{
						if (item2.BoeID.HasValue && item2.BoeID > 0)
						{
							...
							data.FillData(new List<OrdinaryVariableDto>() { new OrdinaryVariableDto() { SelectedBOEsToSum = new Collection<SelectBOEsToSum>() { new SelectBOEsToSum() { BoeID = item2.BoeID } } } },
								new List<WorkspaceVariableDTO>() { new WorkspaceVariableDTO() { SelectedBOEsToSum = new Collection<SelectBOEsToSum>() { new SelectBOEsToSum() { BoeID = item2.BoeID } } } },
								exportInputs.WbsElements, exportInputs.AllWorkspaceBoes, exportInputs.TaskElements, exportInputs.ResourcesForWsResourceListId, exportInputs.Clins);
							ovboemv.Total = variableSelectBOEtoSumCalculation.GetTotalBasedOnBoeID(item2.BoeID.Value, item.SumVariableResourceTypeIDs, data);
							ovmv.ReferencedBOEs.Add(ovboemv);
						}
					}
		...
		public void SetWorkspacePrecisionVariables(WorkspaceDTO ws)
		{
			...
			if (CommonUtilities.IsUCOTEnabled)
			{
		...
		/// <returns></returns>
		public static string GetRteOverride(int boeId, int? taskId, string original, IES.Common.Core.Enums.RteTemplateSource source, IReadOnlyCollection<RTECustomTemplateQuestionAnswerModelView> answers)
		{
