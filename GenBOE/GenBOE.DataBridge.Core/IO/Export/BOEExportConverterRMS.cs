...
namespace GenBOE.DataBridge.Core.IO.Export
{
	...
	/// </summary>
	/// <seealso cref="BOEExportConverter" />
	public class BOEExportConverterRMS : BOEExportConverter
	{
		...
		public BOEExportConverterRMS(ICommonDataMapper commonDataMapper, IVariableSelectBOEtoSumCalculation variableSelectBOEtoSumCalculation,
			...
			RMSZoneTravelRatesFeesDataLoader rmsZoneTravelRatesFeesDataLoader, ILogger<BOEExportConverterRMS> logger)
					: base(logger, commonDataMapper, variableSelectBOEtoSumCalculation, travelTripCostCalculation)
		{
		...
		protected override void GetCompanySpecificTaskCustomFields(IReadOnlyCollection<CustomFieldDTO> workspaceCustomFields, 
			IReadOnlyCollection<CustomFieldValueDTO> workspaceCustomFieldValues, BoeTaskElementDTO boeTaskElement, 
			BOEExportTaskElement boeExportTaskElement)
		{
			if (boeExportTaskElement != null)
			{
				// get Cost Center Custom fields for MST Templates
				setTaskCustomField(workspaceCustomFields, workspaceCustomFieldValues, boeTaskElement, boeExportTaskElement, "Cost Center", BOEExporter.FieldName_TaskCostCenter, true);

				// get Estimate Method Custom Fields
				setTaskCustomField(workspaceCustomFields, workspaceCustomFieldValues, boeTaskElement, boeExportTaskElement, BOEExporterConstants.CustomFieldName_EstimateMethod, BOEExporter.FieldName_EstimateMethod, false);

				// get Company Custom Fields
				setTaskCustomField(workspaceCustomFields, workspaceCustomFieldValues, boeTaskElement, boeExportTaskElement, BOEExporterConstants.CustomFieldName_Company, BOEExporter.FieldName_Company, false);
			}
		}
		...
		protected override void PopulateTravelTasks(BoeDTO boe, BOEExportInputs exportInputs, bool writelogstatements, Collection<BOEExportTaskElement> boeExportTaskElements)
		{
			...
			foreach (TravelDTO travelElement in travelTasks)
			{
				...
				foreach (MSTTravelTripType travelTrip in travelElement.MSTTravelTrips)
				{
					...
					{
						boeExportLabor.Cost = CalculateNonzoneTravelTripCost(exportInputs, travelTrip, escalationRates, fees);
					...
					boeExportLabor.ExportFields[BOEExporter.FieldName_TaskTypePeople] = CommonUtilities.FormatStringWithPrecision(people, exportInputs.Workspace.DecimalPrecision);
					boeExportLabor.ExportFields[BOEExporter.FieldName_TaskTypeDays] = CommonUtilities.FormatStringWithPrecision(days, exportInputs.Workspace.DecimalPrecision);
					boeExportLabor.ExportFields[BOEExporter.FieldName_TaskTypeCars] = travelTrip.NonZoneNumCars.HasValue ? CommonUtilities.FormatStringWithPrecision(travelTrip.NonZoneNumCars.Value, exportInputs.Workspace.DecimalPrecision) : "0";
