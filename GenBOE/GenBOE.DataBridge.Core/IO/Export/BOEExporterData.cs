...
namespace GenBOE.DataBridge.Core.IO.Export
{
	...
	public class ResourceSummaryRowData
	{
		...
		/// </summary>
		public string GroupKey { get { return $"{ResourceType}::{ResourceName}"; } }

		/// <summary>
		/// Group key for grouping resources including Labor Category and Location
		/// </summary>
		public string GroupKeyWithLaborCategoryAndLocation { get { return $"{ResourceType}::{ResourceName}::{LaborCategory}::{Location}"; } }
	}
	...
	public class RollupSummaryByYearTableRowData : ICloneable
	{
		...
		public object Clone()
		{
			RollupSummaryByYearTableRowData clone = new RollupSummaryByYearTableRowData
			{
				Year = Year,
				YearTotal = YearTotal,
				MonthlyValues = MonthlyValues.Clone() as ValuesByMonth<decimal>
			};
	...
	public class LaborRollupByDateNew : ICloneable
	{
		...
		public LaborRollupByDateNew(int year, ValuesByMonth<decimal> monthlyValues)
		{
			...
			January = monthlyValues.January;
			February = monthlyValues.February;
			March = monthlyValues.March;
			April = monthlyValues.April;
			May = monthlyValues.May;
			June = monthlyValues.June;
			July = monthlyValues.July;
			August = monthlyValues.August;
			September = monthlyValues.September;
			October = monthlyValues.October;
			November = monthlyValues.November;
			December = monthlyValues.December;
			...
			if (resourcesData != null)
			{
				foreach (BOEExportTaskElementLabor resource in resourcesData)
				{
					...
					laborResourcesData.Add(new LaborResourceTypesTableRowData
					{
						...
						Hours = CommonUtilities.FormatStringWithPrecision(hours, decimalPrecision),
