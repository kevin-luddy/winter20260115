...
namespace GenBOE.DataBridge.Core.IO.Export
{
	...
	public class BOECustomExporter : WordExporter, IBOECustomExporter
	{
			...
			if (boeExportModelViews.Any() || boeSummaryGridModelViews.Any())
			{
				...
				foreach (BOEExportModelView boeExportModelView in boeExportModelViews)
				{
					...
					try  // catch and rethrow to give exception some context
					{
						...
						ProcessResourceSummaryByResourceTypeTable(boeContainer, boeSummaryGridModelView, selectedComponents, exportInputs);
						ProcessResourceSummaryByResourceIDTable(boeContainer, boeExportModelView, selectedComponents, exportInputs);
						ProcessResourceSummaryByElementOfCostTable(boeContainer, boeExportModelView, boeSummaryGridModelView, selectedComponents, exportInputs);
		...
		private void ProcessResourceSummaryByResourceTypeTable(StructuredDocumentTag boeContainer, ICollection<BOESummaryGridModelView> boeSummaryGridModelView, ICollection<BoeCustomReportComponent> selectedComponents, BOEExportInputs exportInputs)
		{
			if (selectedComponents == null)
			{
				throw new ArgumentNullException(nameof(selectedComponents));
			...
			{
				PopulateResourceSummaryByResourceTypeTable(resourceSummaryByResourceTypeTableElement, boeSummaryGridModelView, exportInputs);
			}
			else
			{
		...
		private void ProcessResourceSummaryByElementOfCostTable(StructuredDocumentTag boeContainer, BOEExportModelView boeExportModelView, ICollection<BOESummaryGridModelView> boeSummaryGridModelView, ICollection<BoeCustomReportComponent> selectedComponents, BOEExportInputs exportInputs)
		{
			if (selectedComponents == null)
			{
				throw new ArgumentNullException(nameof(selectedComponents));
			...
			{
				PopulateResourceSummaryByElementOfCostTable(resourceSummaryByElementOfCostTableElement, boeExportModelView, boeSummaryGridModelView, exportInputs);
			}
			else
			{
		...
		private void ProcessResourceSummaryByResourceIDTable(StructuredDocumentTag boeContainer, BOEExportModelView boeExportModelView, ICollection<BoeCustomReportComponent> selectedComponents, BOEExportInputs exportInputs)
		{
			if (selectedComponents == null)
			{
				throw new ArgumentNullException(nameof(selectedComponents));
			...
			if (resourceSummaryByResourceIDTableElement != null)
			{
				...
				{
					PopulateResourceSummaryByResourceIDTable(resourceSummaryByResourceIDTableElement, boeExportModelView, exportInputs);
				}
				else
				{
		...
		protected virtual void PopulateResourceSummaryByElementOfCostTable(StructuredDocumentTag tableContainerElement, BOEExportModelView boeExportModelView, ICollection<BOESummaryGridModelView> data, BOEExportInputs exportInputs)
		{
			if (data.Any())
			{
		...
		private void PopulateResourceSummaryByResourceTypeTable(StructuredDocumentTag tableContainerElement, ICollection<BOESummaryGridModelView> data, BOEExportInputs exportInputs)
		{
			if (data.Any())
			{
		...
		/// <param name="exportInputs">Export inputs</param>
		protected virtual void PopulateResourceSummaryByResourceIDTable(StructuredDocumentTag tableContainerElement, BOEExportModelView boeExportModelView, BOEExportInputs exportInputs)
		{
			if (boeExportModelView != null)
			{
		...
		/// <param name="exportInputs">Export inputs</param>
		protected virtual void PopulateResourceSummaryTable(StructuredDocumentTag tableContainerElement, ICollection<ResourceSummaryRowData> rollupData, BOEExportInputs exportInputs)
		{
		...
		/// <param name="useGfy">use government fiscal year?</param>
		/// <param name="includeUcot">Should UCOT resources be included in the rollup</param>
		/// <returns>A <see cref="LaborRollupByDateNew"/> object containing the rolled up data</returns>

		protected virtual List<LaborRollupByDateNew> GetRollupByYear(ICollection<BoeTaskElementDTO> taskElementCollection, Collection<ResourceDTO> resources = null, RateType? rateTypeFilter = null, bool useGfy = false, bool includeUcot = false)
		{
			...
			else
			{
				...
				allTaskElementResources = taskElementCollection.SelectMany(x => x.taskElementLabors).Where(r => r.ResourceID.HasValue
					&& (resourceIds.Contains(r.ResourceID.Value) || (includeUcot && r.ResourceID <= CommonConstants.UCOT_RESOURCE_ID))).ToList().DeepClone();
			}
			...
			if (Earliest != null && Latest != null)
			{
				for (int i = Earliest.StartDate.Value.Year; i <= Latest.EndDate.Value.Year; i++)
				{
					...
					Rollup.January = this.GetRollupForMonth(allTaskElementResources, i, 1, resources, rateTypeFilter, includeUcot);
					Rollup.February = this.GetRollupForMonth(allTaskElementResources, i, 2, resources, rateTypeFilter, includeUcot);
					Rollup.March = this.GetRollupForMonth(allTaskElementResources, i, 3, resources, rateTypeFilter, includeUcot);
					Rollup.April = this.GetRollupForMonth(allTaskElementResources, i, 4, resources, rateTypeFilter, includeUcot);
					Rollup.May = this.GetRollupForMonth(allTaskElementResources, i, 5, resources, rateTypeFilter, includeUcot);
					Rollup.June = this.GetRollupForMonth(allTaskElementResources, i, 6, resources, rateTypeFilter, includeUcot);
					Rollup.July = this.GetRollupForMonth(allTaskElementResources, i, 7, resources, rateTypeFilter, includeUcot);
					Rollup.August = this.GetRollupForMonth(allTaskElementResources, i, 8, resources, rateTypeFilter, includeUcot);
					Rollup.September = this.GetRollupForMonth(allTaskElementResources, i, 9, resources, rateTypeFilter, includeUcot);
					Rollup.October = this.GetRollupForMonth(allTaskElementResources, i, 10, resources, rateTypeFilter, includeUcot);
					Rollup.November = this.GetRollupForMonth(allTaskElementResources, i, 11, resources, rateTypeFilter, includeUcot);
					Rollup.December = this.GetRollupForMonth(allTaskElementResources, i, 12, resources, rateTypeFilter, includeUcot);
		...
		private decimal GetRollupForMonth(List<ResourceTypeDto> resourceTypeCollection, int year, int month, Collection<ResourceDTO> resources, RateType? rateTypeFilter, bool includeUcot)
		{
			decimal sum;
			...
			else
			{
				...
				sum =
					(from f in resourceTypeCollection
					 where f.SpreadType == SpreadType.Hours && (resourceIds.Contains(f.ResourceID.Value) || (includeUcot && f.ResourceID.Value <= CommonConstants.UCOT_RESOURCE_ID))
					 from g in f.LaborSpreads
					 where g.LaborSpreadDate.Year == year && g.LaborSpreadDate.Month == month
					 select g.LaborSpreadValue).Sum();
			}
