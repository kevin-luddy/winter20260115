...
namespace IES.Tests.Core
{
    ...
    public class SectionLoaderTest
    {
        ...
        {
            ISectionLoader sut = this.testData.SectionLoader;
		...
		public void ValidateSectionLoaderUpdateSectionsAndContentAddSection()
        {
            ...
            SectionModelView sectionFromGui = new()
			{
                Id = -1,
		...
		public void TestRetrievingSubSetOfSections()
        {
            ...
            ICollection<SectionModelView> subset = sut.GetAll(revision, false, true, new int[] { section1.Id, section11.Id, section112.Id });
		...
		{
			SectionLoader sut = this.testData.SectionLoader;
			RDSBCoverSheetDataModelView result = sut.GetCoverSheetData(15204);

            Assert.AreEqual(null, result.CasbSection);
            Assert.AreEqual(null, result.NonComplianceSection);
            Assert.AreEqual(null, result.AdequateDisclosure);
        ...
        {
            SectionLoader sut = this.testData.SectionLoader;
			RDSBCoverSheetDataModelView result = sut.GetCoverSheetData(17718);
