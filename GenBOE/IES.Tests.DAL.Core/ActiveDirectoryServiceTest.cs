...
namespace IES.Tests.Core
{
	...
	public class ActiveDirectoryServiceTest
	{
		...
		{
			ActiveDirectoryService sut = new(Mock.Of<ILogger<ActiveDirectoryService>>(), Mock.Of<ICacheService>(), 30);
		...
		{
			ActiveDirectoryService sut = new(Mock.Of<ILogger<ActiveDirectoryService>>(), Mock.Of<ICacheService>(), 30, 300);
		...
		{
			ActiveDirectoryService activeDirectory = new(Mock.Of<ILogger<ActiveDirectoryService>>(), Mock.Of<ICacheService>(), 0);
		...
		{
			// test searching for an ntid that exists in the AD
			ActiveDirectoryService sut = new(Mock.Of<ILogger<ActiveDirectoryService>>(), Mock.Of<ICacheService>(), 0, 300);
		...
		{
			ActiveDirectoryService sut = new(Mock.Of<ILogger<ActiveDirectoryService>>(), Mock.Of<ICacheService>(), 0);
		...
		public void TestGetUserAndGroupIdsAsXml()
		{
			...
			string ntid = "ejfudd";
			ICollection<GroupData> groups = null;
