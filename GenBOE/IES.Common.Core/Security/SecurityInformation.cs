...
namespace IES.Common.Core.Security
{
	...
	{
		private readonly IActiveDirectoryService _ADUtils;
		...
		/// </summary>
		public SecurityInformation(IActiveDirectoryService inADUtils, ICacheService memCache,
		...
		virtual public UserData ActiveUserData
		{
			get
			{
				return _ADUtils.GetUserByQualifiedAccount(ActiveUserNTID, false);
			}
		}
		...
		protected virtual string ActiveUserNTIDWithDomain
		{
			get
			{
				...
				{
					ntid = httpContextAccessor.HttpContext.User.Identity.Name;
				}
		...
		virtual public bool IsSubcontractorUser(string ntid, bool? isSubcontractor)
		{
			...
			bool? resultFromCache = IsUserInCacheCheck(ntid, cacheKeyIsSubcontractorByUserName);
			...
			if ((bool)isSubcontractor)
			{
				try
				{
					isTreatedAsLmEmployee = IsMemberOfADGroupInAppSettingsList(ntid, "SubsTreatedAsLmEmployees");
				}
			...
			AddResultToCache(ntid, cacheKeyIsSubcontractorByUserName, secondsToCacheIsSubcontractor, isSub);
		...
		virtual public bool IsRdmAdminUser(string inUserName)
		{
			...
			bool? resultFromCache = IsUserInCacheCheck(inUserName, cacheKeyIsRdmAdminByUserName);
			if (resultFromCache.HasValue)
			{
				return resultFromCache.Value;
			}

			bool isRDMAdminUser = IsMemberOfADGroupInAppSettingsList(inUserName, "RDMAdminGroups");
			AddResultToCache(inUserName, cacheKeyIsRdmAdminByUserName, secondsToCacheIsRdmAdmin, isRDMAdminUser);
		...
		virtual public bool IsRdmCobraAdminUser(string inUserName)
		{
			...
			bool? resultFromCache = IsUserInCacheCheck(inUserName, cacheKeyIsRdmCobraAdminByUserName);
			if (resultFromCache.HasValue)
			{
				return resultFromCache.Value;
			}

			bool isRDMCobraAdminUser = IsMemberOfADGroupInAppSettingsList(inUserName, "RDMCobraAdminGroups");
			AddResultToCache(inUserName, cacheKeyIsRdmCobraAdminByUserName, secondsToCacheIsRdmCobraAdmin, isRDMCobraAdminUser);
		...
		virtual public bool IsIESPortalAdminUser(string inUserName)
		{
			...
			bool? resultFromCache = IsUserInCacheCheck(inUserName, cacheKeyIsIESPortalAdminByUserName);
			if (resultFromCache.HasValue)
			{
				return resultFromCache.Value;
			}

			bool isIESPortalAdminUser = IsMemberOfADGroupInAppSettingsList(inUserName, "IESPortalAdminGroups");
			AddResultToCache(inUserName, cacheKeyIsIESPortalAdminByUserName, secondsToCacheIsIESPortalAdmin, isIESPortalAdminUser);
		...
		virtual public bool IsRdmViewerUser(string inUserName)
		{
			...
			bool? resultFromCache = IsUserInCacheCheck(inUserName, cacheKeyIsRdmViewerByUserName);
			if (resultFromCache.HasValue)
			{
				return resultFromCache.Value;
			}

			bool isRDMViewerUser = IsMemberOfADGroupInAppSettingsList(inUserName, "RDMViewerGroups");
			AddResultToCache(inUserName, cacheKeyIsRdmViewerByUserName, secondsToCacheIsRdmViewer, isRDMViewerUser);
		...
		virtual public bool IsAllowedProPricerAccess(string userName)
		{
			...
			bool? resultFromCache = IsUserInCacheCheck(userName, cacheKeyIsAllowedProPricerAccess);
			if (resultFromCache.HasValue)
			{
				return resultFromCache.Value;
			}
			...
			bool isAllowedProPricerAccess =
				IsMemberOfADGroupInAppSettingsList(userName, "UsersAllowedProPricerAccess");
			AddResultToCache(userName, cacheKeyIsAllowedProPricerAccess, secondsToCacheIsAllowedProPricerAccess, isAllowedProPricerAccess);
		...
		private bool? IsUserInCacheCheck(string userName, string cacheKey)
		{
			...
			if (cache.Contains(cacheKey + userName))
			{
				result = (bool)cache.GetData(cacheKey + userName);
			}
		...
		{
			cache.Add(cacheKey + userName, result, secondsToCache);
		}
