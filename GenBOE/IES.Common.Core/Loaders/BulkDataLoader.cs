...
namespace IES.Common.Core.Loaders
{
		...
		where TDtoType : IUpdateableDTO
	{
		...
		protected BulkDataLoader(ILogger logger) : base(logger)
		{ }
		...
		public virtual IDictionary<int, int> BulkSave(ICollection<TDtoType> dtosToSave)
		{
			...
			using (StopwatchTimer sw = new StopwatchTimer(Log))
			{
				...
				if (dtosToSave.Any(d => d.Updateable != UpdateType.None))
				{
					...
					BulkSaveMetaData metadata = CreateBulkSaveMetaData();
					...
					{
						updateResults = BulkUpdate(updateCollection, metadata);
					}
					...
					{
						BulkDelete(deleteCollection, metadata);
					}

					if (insertCollection != null && insertCollection.Any() && loaderSupportsBulkInsert)
					{
						insertResults = BulkInsert(insertCollection, metadata);
					}
		...
		protected virtual Dictionary<int, int> BulkInsert(ICollection<TDtoType> dtosToInsert, BulkSaveMetaData metadata)
		{
			...
			if (dtosToInsert.Any())
			{
				...
				Log.LogDebug("Starting entity conversion for BulkInsert.");
				ICollection<TEntityType> entitiesToInsert = dtosToInsert.Select(d => ConvertDtoToEntity(d)).ToCollection();
				...
				using (DbContext objectContext = new DbContext(metadata.DbContextName))
				{
					...
					try
					{
						Log.LogDebug("Start Insert: " + DateTime.UtcNow.ToString("yyyy-MM-dd HH:mm:ss.fff", CultureInfo.InvariantCulture));
						...
						Log.LogDebug("End Insert: " + DateTime.UtcNow.ToString("yyyy-MM-dd HH:mm:ss.fff", CultureInfo.InvariantCulture));
					}
					catch (Exception ex)
					{
						Log.LogError(ex, $"Executing {metadata.BulkInsertStoredProcedureName} failed for {metadata.DBTableTypeName}");
						throw;
					}

					Log.LogDebug($"Updating {dtosToInsert.Count} child DTOs for new Ids.");
					...
					Parallel.For(0, dtosToInsert.Count, i =>
					{
						...
						{
							Log.LogError(ex, $"Update failed for item {i} with message {ex.Message}, originalId {origIds.ElementAt(i)}, key {dtosToInsert.ElementAt(i)?.Id}.");
							throw;
						}
					});

					Log.LogDebug("Child DTO updates complete.");
				}
			}

			Log.LogDebug("Bulk Insert complete.");
		...
		protected virtual Dictionary<int, int> BulkUpdate(ICollection<TDtoType> dtosToUpdate, BulkSaveMetaData metadata)
		{
			...
			Log.LogDebug("Bulk Update Complete.");
		...
		protected virtual void BulkDelete(ICollection<TDtoType> dtosToDelete, BulkSaveMetaData metadata)
		{
			...
			if (dtosToDelete.Any())
			{
				ICollection<TEntityType> entitiesToDelete = dtosToDelete.Select(d => ConvertDtoToEntity(d)).ToCollection();
