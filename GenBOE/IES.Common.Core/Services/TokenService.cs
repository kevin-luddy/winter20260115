...
namespace IES.Common.Core.Services
{
	...
	public class TokenService : ITokenService, IDisposable
	{
		...
		/// </summary>
		internal readonly ICacheService cache;
		...
		public TokenService(ILogger<TokenService> logger, ICacheService memoryCache, IConfiguration configuration)
		{
			_log = logger;
			cache = memoryCache;
		...
		public async Task<Token> GetToken()
		{
			...
			if (!cache.Contains(CACHE_KEY_TOKEN))
			{
				...
				cache.AddAbsolute(CACHE_KEY_TOKEN, token, 5 * 60);
			}
			else
			{
				token = cache.GetData(CACHE_KEY_TOKEN) as Token;
			}
		...
		private async Task<Token> CreateToken()
		{
			...
			{
				_log.LogError("Error creating new Token.");
