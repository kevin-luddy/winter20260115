...
namespace IES.Common.Core.Services
{
	...
	public class ActiveDirectoryService : IActiveDirectoryService
	{
		...
		public ActiveDirectoryService(ILogger<ActiveDirectoryService> logger, ICacheService cache, int inSecondsToCache = 720, int clientTimeoutSeconds = -1)
		{
			log = logger;
			this.cache = cache;
			secondsToCache = inSecondsToCache;
		...
		public virtual UserData GetUserByQualifiedAccount(string inNtid, bool isGroup)
		{
			...
			{
				object fromCache = cache.GetData(cacheKey);
				log.LogDebug("ADUTILS - UserData from temporary cache is " + fromCache.ToString());
			...
			else
			{
				...
				{
					log.LogDebug("GET USER FROM AD : For user Ntid " + cacheKey + " caching their data for " + secondsToCache + " seconds [" + currentAccount + "]");
					cache.Add(cacheKey, currentAccount, secondsToCache); // cache for this.secondsToCache seconds
				}
			}
			...
			if (fromCache == null)
			{
				lock (string.Intern(cacheKey))
				{
					if (!cache.Contains(cacheKey))
					{
						...
						List<GroupData> toCache = distinctGroups.Values.ToList();


						cache.Add(cacheKey, toCache, secondsToCache); // cache for this.secondsToCache seconds
					}
				}

				fromCache = cache.GetData(cacheKey);
			}
		...
		public bool IsValidADGroup(string inGroupName)
		{
			...
			if (cache.Contains(cacheKey))
			{
				return (bool)cache.GetData(cacheKey);
			}
			...
			cache.Add(cacheKey, valid, secondsToCache); // cache for this.secondsToCache seconds
		...
		public bool IsMemberOfADGroup(string inUserName, string inGroupName)
		{
			...
			ICollection<GroupData> groupData = GetGroupsForUser(inUserName);
		...
		public Dictionary<UserData, bool> CheckUsersBoeAccess(ICollection<UserData> usersToCheck, ICollection<GroupData> groupsToCheckAgainst)
		{
			...
			foreach (GroupData individualGroup in groupsToCheckAgainst)
			{
				try
				{
					ICollection<UserData> userNames = GetAdGroupUsers(individualGroup.Ntid);
		...
		public ICollection<UserData> GetAdGroupUsers(string inGroupName)
		{
			...
			{
				userNames = cache.GetData(key) as ICollection<UserData>;
			}
			else
			{
				int tries = 0;
				...
				if (userNames != null)
				{
					...
					cache.Add(key, userNames, secondsToCache);
				}
			}
		...
		private Dictionary<string, Dictionary<string, string>> GetActiveDirectoryProperties(SearchResultCollection results, string[] propertyNames)
		{
			...
			{
				Dictionary<string, string> properties = GetActiveDirectoryProperties(res, propertyNames);
		...
		public ICollection<UserData> SearchUsers(string userSearchString, ActiveDirectorySearchBy searchBy, ActiveDirectoryMatchType matchBy)
		{
			...
			if (!string.IsNullOrEmpty(userSearchString))
			{
				...
				SearchResultCollection searchResults = FindMatchingUsers(sanitizedString, searchBy, matchBy, propertyNames);
				...
				if (searchBy == ActiveDirectorySearchBy.Account)
				{
					...
					searchResults = FindMatchingGroups(sanitizedString, matchBy);
		...
		private List<UserData> CreateUserDtosFromAdData(string[] propertyNames, SearchResultCollection matchSet, bool isUserNotGroup)
		{
			...
			foreach (KeyValuePair<string, Dictionary<string, string>> userProperties in propertiesByUser)
			{
				...
				result.Add(new UserData
				{
					...
					Country = GetPropertyValue("c", adproperties),
					Title = GetPropertyValue("title", adproperties),
		...
		public bool IsGroup(string inNtID)
		{
			...
			SearchResultCollection match = FindMatchingGroups(inNtID, ActiveDirectoryMatchType.Exact);

			Dictionary<string, Dictionary<string, string>> propertiesByUser = GetActiveDirectoryProperties(match, propertyNames);

			foreach (KeyValuePair<string, Dictionary<string, string>> userProperties in propertiesByUser)
			{
				Dictionary<string, string> adproperties = userProperties.Value;

				string objectCategory = GetPropertyValue("objectCategory", adproperties);
				string accountType = GetAccountTypeFromObjectCategory(objectCategory);
