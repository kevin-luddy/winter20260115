...
namespace IES.Common.Core.OfficeUtilities
{
	...
	public static class WordUtilities
	{
		...
		/// <summary>
		/// First part of the Resource Summary UCOT Label Text
		/// </summary>
		public const string OPENING_UCOT_LABEL_TEXT = "** The hours generated by the Uncompensated OT HEF are included in the Resource IDs appended with “-UCOT” and are in addition to the hours calculated within the respective actuals based MOQ Equation(s).";

		/// <summary>
		/// Starting last part of the Resource Summary UCOT Label Text
		/// </summary>
		public const string START_CLOSING_UCOT_LABEL_TEXT = " See ";

		/// <summary>
		/// Middle, bolded part of the Resource Summary UCOT Label Text
		/// </summary>
		public const string BOLD_UCOT_LABEL_TEXT = "UCOT HEF - genBOE";

		/// <summary>
		/// Ending last part of the Resource Summary UCOT Label Text
		/// </summary>
		public const string END_CLOSING_UCOT_LABEL_TEXT = " section of the Cost Volume for additional details.";
		...
		/// <summary>
		/// Add the UCOT notice label to the bottom of the given container element
		/// </summary>
		/// <param name="containerElement">Container element to add label to</param>
		/// <param name="includeCostVolumeUCOTText">Indicates if to include Cost Volume UCOT Text in export</param>
		public static void AddUcotLabelToContainer (StructuredDocumentTag containerElement, bool includeCostVolumeUCOTText)
		{
			if (containerElement != null)
			{
				...
				if (includeCostVolumeUCOTText)
				{
					Run startClosingRun = new Run(doc, START_CLOSING_UCOT_LABEL_TEXT);
					SetRunFont(startClosingRun, font, false);

					Run boldRun = new Run(doc, BOLD_UCOT_LABEL_TEXT);
					SetRunFont(boldRun, font, true);

					Run endClosingRun = new Run(doc, END_CLOSING_UCOT_LABEL_TEXT);
					SetRunFont(endClosingRun, font, false);

					paragraph.AppendChild(startClosingRun);
					paragraph.AppendChild(boldRun);
					paragraph.AppendChild(endClosingRun);
				}
			}
		}

		/// <summary>
		/// Set the font properties of a Run since they need to be set individually
		/// </summary>
		/// <param name="run">Run to set the font of</param>
		/// <param name="font">Font to apply to the run</param>
		/// <param name="isBold">If the text should be made bold</param>
		private static void SetRunFont(Run run, Font font, bool isBold)
		{
		...
		public static void SetElementTextWithHTML(Document document, StructuredDocumentTag element, string htmlFormattedText, bool removeSpacing = false, bool keepParagraphs = true)
		{
			if (element != null)
			{
				...
				if (!string.IsNullOrEmpty(htmlFormattedText))
				{
					...
					if (element.Level == MarkupLevel.Inline)
					{
						if (element.ParentNode is Paragraph paragraph)
						{
							SplitParagraphForSDT(document, paragraph, element, htmlFormattedText, options);
						}
						else if (element.ParentNode is StructuredDocumentTag tag && element.ParentNode.ParentNode is Paragraph para)
						{
							SplitSDTForSDT(document, para, tag, element, htmlFormattedText, options);
						}
						else
						{
							InsertHtmlIntoSDTBlock(document, element, htmlFormattedText, options);
						}
					}
					else
					{
		...
		private static void InsertHtmlIntoSDTBlock(Document document, StructuredDocumentTag element, string htmlFormattedText, HtmlInsertOptions htmlInsertOptions)
		{
			...
			{
				// backup for inline, if there is a table then it will crash
				element.AppendChild(new Run(document));
			}

			DocumentBuilder builder = new DocumentBuilder(document);
			builder.MoveTo(element);
				...
				try
				{
					...
					foreach (Node child in children) 
					{
						if (child == tag)
						{
							...
							// Create a fake Inline Tag to insert other children into
							splitParent = new StructuredDocumentTag(document, tag.SdtType, MarkupLevel.Inline);
							splitParent = parentParagraph.InsertAfter(splitParent, tag);
						}
						else if (foundSDT)
						{
					...
					// Clean up empty original SDT if no children
					if (parent != null && !parent.HasChildNodes)
					{
						parent.Remove();
					}

					// Clean up empty split SDT if no children
					if (splitParent != null && !splitParent.HasChildNodes)
					{
						splitParent.Remove();
					}
		...
		private static void SplitParagraphForSDT(Document document, Paragraph parent, StructuredDocumentTag tag, string htmlFormattedText, HtmlInsertOptions htmlInsertOptions)
		{
			...
			Paragraph currentParagraph = parent;
			Node lastInsertedSibling = currentParagraph;
			try
			{
				bool foundSDT = false;
				...
				foreach (Node child in children) 
				{
					if (child == tag)
					{
						...
						// Create a fake Block Tag and insert html into it
						StructuredDocumentTag blockTag = new StructuredDocumentTag(document, SdtType.RichText, MarkupLevel.Block);
						blockTag = parent.ParentNode.InsertAfter(blockTag, parent);
						builder.MoveTo(blockTag);
						builder.InsertHtml(htmlFormattedText, htmlInsertOptions);

						// remove the old SDT
						tag.Remove();

						// Create an ending Paragraph for any later children
						splitParagraph = new Paragraph(document);
						parent.ParentNode.InsertAfter(splitParagraph, blockTag);
					}
					else if (foundSDT)
					{
				...
				if (splitParagraph != null && !splitParagraph.HasChildNodes)
				{
					splitParagraph.Remove();
				}
			}
			catch (Exception ex)
			{
				Serilog.Log.Error(ex, "Error moving SDT outside of Paragraph");
			}
		}
