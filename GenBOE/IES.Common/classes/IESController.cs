...
namespace IES.Common
{
    ...
    public class IESController : Controller
    {
        ...
        /// The logger.
        /// </summary>
        private Logger log = new Logger(typeof(IESController));
        ...
        private string ConvertImageSrcAttribute(string src)
        {
            ...
            else if (Uri.TryCreate(src, UriKind.RelativeOrAbsolute, out uri))
            {
                ...
                {
                    this.log.Warn($"Could not retrieve image at \"{src}\".");
        ...
        [NonAction]
        protected Stopwatch StartAction(Logger logger, string functionName)
        {
            if (logger == null)
            {
        ...
        /// <param name="functionName">The function name being logged</param>
        [NonAction]
        protected void FinalizeAction(Logger logger, string functionName)
        {
            if (logger == null)
            {
                throw new ArgumentNullException(nameof(logger));
            ...
            logger.Info(string.Format("Finished " + functionName + ": " + elapsedMilliseconds + " milliseconds."));
            logger.Performance("ACTION - " + functionName, elapsedMilliseconds);
        }
    }
}
