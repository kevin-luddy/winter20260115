...
namespace IES.Common.OfficeUtilities
{
    ...
    public static class WordUtilities
    {
		...
		public static void AddUcotLabelToContainer(SdtElement containerElement, bool includeCostVolumeUCOTText)
		{
			if (containerElement != null)
			{
				SdtContentBlock containerContentBlock = containerElement.Descendants<SdtContentBlock>().FirstOrDefault();
				...
				if (containerContentBlock != null)
				{
					...
					{
						// If no run properties in the paragraph, attempt to get them from the container element
						runProperties = containerElement.Descendants<RunProperties>().LastOrDefault();
					}
					...
					if (includeCostVolumeUCOTText)
					{
						...
						Run startClosingRun = new Run();
						startClosingRun.Append(new Text(START_CLOSING_UCOT_LABEL_TEXT) { Space = SpaceProcessingModeValues.Preserve });
						OpenXmlElement startClosingRunProperties = GetAndProcessRunProperties(runProperties, false);
						startClosingRun.PrependChild(startClosingRunProperties);
						...
						// create the run for the remaining text
						Run endClosingRun = new Run();
						endClosingRun.Append(new Text(END_CLOSING_UCOT_LABEL_TEXT) { Space = SpaceProcessingModeValues.Preserve });
						OpenXmlElement endClosingRunProperties = GetAndProcessRunProperties(runProperties, false);
						endClosingRun.PrependChild(endClosingRunProperties);

						labelParagraph.Append(startClosingRun);
						labelParagraph.Append(boldRun);
						labelParagraph.Append(endClosingRun);
					}
