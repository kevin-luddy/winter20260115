...
/*
    This file was auto-generated for Release: 1.3, on 6/12/2017.
...
/*
    File: \Release 1.3\Release 1.3 Script.sql
*/
PRINT '### Starting file: \Release 1.3\Release 1.3 Script.sql';
/*
	## START ##
...
--	'$' + REPLACE(CONVERT(varchar,CAST(PC.ProfitFee AS MONEY),1), '.00','') AS [Profit/Fee + COM],	
--	'$' + REPLACE(CONVERT(varchar,CAST(PC.TotalPrice AS MONEY),1), '.00','') AS [Total Price],
...
/*
	...
	SET	@SpecificProposals = 0
	SET @Year ='ALL'--'2013'--IF SPECIFIC IS 0, THESE WILL BE SENT AS NULL
	SET @LOB = '1,2,3'  /*Evaluates as Civil - Energy & Environmental Services*/


	
	SET @LeadEstimator  = 'ALL'
	
	SET @SubmitStartDate  = NULL
	SET @SubmitEndDate  = NULL
...
/*	LEFT OUTER JOIN @MaxRev M ON LEFT(V.[Tracking #],10) = M.MainProposalTrackingID*/
...
WHERE
	...
	(
		...
		(
			...
			/*
			DateCreated > = @SubmitStartDate AND
			DateCreated < = @SubmitEndDate
			*/
...
/*
    File: \Stored Procedures\CreateProposalLogReportByLatestVersion.proc.sql
*/
PRINT '### Starting file: \Stored Procedures\CreateProposalLogReportByLatestVersion.proc.sql';
...
CREATE  PROCEDURE [dbo].[CreateProposalLogReportByLatestVersion]
(
	@SubmitStartDate [date]=NULL,
	@SubmitEndDate [date]=NULL
)	
...
/******************************************************************************
**		 
**		Name: [CreateProposalLogReportByLatestVersion]
**		Desc: SSRS: Proposal Log Report
**			
**		
**
...
**		8/28/13		dcanuso				Fields added
**		11/20/13	dcanuso				Removed: LMISTotalPrice
**		12/4/13		dcanuso				Stored Procedure created from Log Report SP
**										New report will run for a Submitted Date Range
...
**										Columns are entered when user
**										selects other from LU table
**		03/03/16	dturk				Added ProgramProposalStatus field to result set per WI 37694
**		1/5/17		Dusan				Removing Revisions and LMIS
**		1/5/17		pattoncr			Removing Segment
**      1/5/2017	twilson3			BOEJ-1706 Remove ICE fields
**		01/19/17	n99040				added fields TechLead, IndependentReviewer, LeadEstimator, ProposalMgr, CoverSheetApprover, PricingVErification
**		01/19/17	n99040				removed code related to table ProposalReorganization
**		02/02/17	tglick				added new fields [Revised Submittal Date], [AbsoluteValue]
**      02/17/17	twilson3			BOEJ-1903 Add LOB Estimating Manager
**										BOEJ-1905 Remove IS&GS from Total Price column
**		3/22/2017	twilson3			BOEJ-1909 Remove Profit Tracking #
...
--set @SubmitStartDate ='1-01-2013'
--set	@SubmitEndDate ='11-30-2013'
...
DECLARE @MaxSubmittedDate TABLE
	(
		ProposalTrackingID char(10),
		SubmittedDate datetime2(7)
	)
INSERT INTO @MaxSubmittedDate
SELECT /*LEFT (P.ProposalTrackingID, 10) AS ProposalTrackingID*/
		CASE LEN(LEFT (ProposalTrackingID, CHARINDEX ('-',ProposalTrackingID) -1))
			WHEN 4 THEN LEFT (P.ProposalTrackingID, 10) 
			WHEN 2 THEN LEFT (P.ProposalTrackingID, 8)
		END AS ProposalTrackingID
      ,MAX([ProposalSubmittalDate])
FROM [dbo].[ProposalChecklist] PC
	INNER JOIN dbo.Proposal P ON PC.ProposalID = P.ProposalID
WHERE
	/*Proposal Status is defined as mandatory*/
	(
		P.ProposalStatusID IN  (2	/*Completed*/, 5	/*Revision*/)
	) AND 
		(
			@SubmitStartDate IS NOT NULL AND
			@SubmitEndDate IS NOT NULL AND
			...
			DateCreated < = @SubmitEndDate
			*/
			CAST(PC.[ProposalSubmittalDate] AS DATE) > = @SubmitStartDate AND
			CAST(PC.[ProposalSubmittalDate] AS DATE) < = @SubmitEndDate			
		) 
/*GROUP BY LEFT (P.ProposalTrackingID, 10)*/
GROUP BY CASE LEN(LEFT (ProposalTrackingID, CHARINDEX ('-',ProposalTrackingID) -1))
					WHEN 4 THEN LEFT (P.ProposalTrackingID, 10) 
					WHEN 2 THEN LEFT (P.ProposalTrackingID, 8)
				END
...
SELECT V.[ProposalID]
	  ...
	  ,V.[AbsoluteValue]
FROM [dbo].[vwProposalLogReport] V
	INNER JOIN @MaxSubmittedDate MSD ON 
/*		LEFT(V.[Tracking #],10) = MSD.ProposalTrackingID AND*/

		(
			(
				LEFT(V.[Tracking #],10) = MSD.ProposalTrackingID AND
				LEN (MSD.ProposalTrackingID) = 10
			) OR
			(
				LEFT(V.[Tracking #],8) = MSD.ProposalTrackingID AND
				LEN (MSD.ProposalTrackingID) = 8
			) 
		) AND
		V.[Submitted Date] = MSD.SubmittedDate
...
GRANT EXECUTE ON OBJECT::dbo.CreateProposalLogReportByLatestVersion TO generationReporter;
GO
...
DECLARE @NTDomain varchar (100) = 'acct04'
DECLARE @genTracUserID INT
*/


IF @genTracUserID IS NULL AND @NTDomain IS NOT NULL AND @NTID IS NOT NULL
BEGIN
	SELECT @genTracUserID = UserID 
	FROM dbo.genTRACUser 
	WHERE 
		NTDomain = @NTDomain AND
		NTID = @NTID
END



IF @genTracUserID IS NOT NULL AND @NTDomain IS NULL AND @NTID IS NULL
BEGIN
	SELECT 
		@NTDomain = NTDomain,
		@NTID = NTID
...
WHERE 
	nt_account_nm = @NTID AND
	nt_domain_nm = @NTDomain
...
DECLARE @ManagerNTDomain varchar (100) = 'acct04'
DECLARE @genTracUserID INT
*/
...
IF @UserID < 0 
	IF EXISTS (SELECT 1 FROM dbo.genTRACUser WHERE NTID = @NTID AND NTDomain = @NTDomain)
				BEGIN

				SET @ErrorMessage =   'A user with this NTID and NTDomain already exists'
				RAISERROR (
					@ErrorMessage, -- Message text.
			        11, -- Severity,/*Severity Changed to 11*/
					1 -- State,
