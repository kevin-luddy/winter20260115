...
set @p_NewCollationName  = (SELECT CONVERT(varchar(100), SERVERPROPERTY('collation'))) --'SQL_Latin1_General_CP1_CI_AS'
...
--Drop Table #Procedures -- (ONLY WITH DEPRECATED COMMANDs)
--  FINISH WORK ON THIS AT ANOTHER TIME

--+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
-- START PROCEDURES -- We only do procedures to get rid of DEPRECATED COMMANDS
--+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
IF (SELECT object_id('tempdb..#Procedures')) IS NOT NULL
...
INSERT INTO #Procedures (SOID, ProcedureName, SchemaName,DEPID, Depth, FText, FtextOrder)
...
AND SO.ID IN (
	SELECT ID 
	FROM syscomments
	WHERE text LIKE '%WITH TRUNCATE_ONLY%'
	)
	
WHILE EXISTS (SELECT * FROM #Procedures a
				inner join #Procedures b
				on a.DepID=b.SOID 
				and a.Depth=b.Depth
				and a.SchemaName + '.' + a.ProcedureName= b.schemaName +'.' + b.ProcedureName
				--order by a.ProcedureName 
				)
	UPDATE A set A.depth=B.Depth+1	-- select *
	FROM #Procedures AS A
	INNER JOIN  #Procedures AS B 
	ON A.DepID=B.SOID
	and a.SchemaName + '.' + a.ProcedureName= b.schemaName +'.' + b.ProcedureName
--	Select * From #Procedures order by ProcedureName

UPDATE P SET Ftext = 
--	SELECT *,
REPLACE(Ftext,'CREATE PROCEDURE','ALTER PROCEDURE')
FROM #Procedures P
UPDATE P SET Ftext = 
--	SELECT *,
REPLACE(Ftext,'BACKUP LOG @DBName WITH TRUNCATE_ONLY','--BACKUP LOG @DBName WITH TRUNCATE_ONLY')
FROM #Procedures P
--get a text pointer
...
declare @ProcedureName sysname,
	@LastProcedureName sysname

IF (SELECT object_id('tempdb..#P')) IS NOT NULL
	DROP TABLE #P;
CREATE TABLE #P (
ID INT IDENTITY (1,1)
,depth INT
...
set 	@LastProcedureName =''
--set @C = cursor for
INSERT INTO #P
	Select max(depth) depth, SOID, ProcedureName, SchemaName, Ftext--, FtextOrder
	From #Procedures --Created earlier in the code, at the DROP ProcedureS LINE
	WHERE SOID IN ( 
		SELECT SOID 
		FROM #Procedures
	)
	group by SOID, SchemaName, ProcedureName, Ftext, FtextOrder 
--	order by ID,depth, SchemaName, ProcedureName, FtextOrder

insert into #SQL (SQL) values ('')

--open @C
--DECLARE @depth INT,@SOID BIGINT,@ProcedureName VARCHAR(50),@SchemaName VARCHAR(50),@SQLSegment VARCHAR(MAX), @counter INT
--fetch next from @C into @depth, @soid, @ProcedureName, @SchemaName, @SQLSegment
DECLARE @schemaName varchar(100)
DECLARE @LastSChemaName VARCHAR(50)
SET @LastSChemaName = ' '
SET @counter = 1
--while @@Fetch_Status=0
WHILE @counter <= ( SELECT MAX(ID) FROM #P )
begin
	SET @ProcedureName = ( SELECT ProcedureName FROM #P WHERE ID = @counter )
	SET @SchemaName = ( SELECT SchemaName FROM #P WHERE ID = @counter )
	SET @SQLSegment = ( SELECT Ftext FROM #P WHERE ID = @counter )
--	PRINT (@sqlsegment)
	if @SchemaName + '.' + @ProcedureName<> @LastSchemaName + '.' + @LastProcedureName AND @LastSChemaName <> ' '
	begin
		--get a text pointer
		SELECT @textptr = TEXTPTR(SQL) FROM #SQL where ID = (select max(ID) from #SQL)
		set @LastProcedureName =@ProcedureName
...
--	declare @cmd nvarchar(max),	DECLARE @counter INT
SET @counter = 1
WHILE @counter <= ( SELECT MAX(ID) FROM #SQL )
BEGIN
	SELECT @cmd = convert(nvarchar(max),SQL) from #SQL WHERE ID = @counter --where len(convert(nvarchar(max),SQL))>1000 or left(convert(nvarchar(max),SQL),13)='DROP FUNCTION'
