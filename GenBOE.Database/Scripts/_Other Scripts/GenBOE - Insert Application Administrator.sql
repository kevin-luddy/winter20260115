-- This Script will setup an application admin and a default test workspace.

-- The application will not work correctly if any of these fields do not match AD
...
-- Insert the user
INSERT INTO [ETIUser] (UpdateDT, NTID, DisplayName, FirstName, LastName, EmailAddress)
	VALUES (GETDATE(), @ntid, @displayName, @firstName, @lastName, @email);

-- Grant System Admin role
INSERT INTO [SystemUserRole] (UpdateDT, ETIUserID, RoleId)
	VALUES (GETDATE(), (SELECT TOP(1) EtiUserId FROM EtiUser), 6);
...
INSERT INTO dbo.Workspace
  ...
  VALUES (1, GETDATE(), 'Test - CIO', 'test_-_cio', 2, '2013-12-15', '2016-12-15', NULL,
   'Test Workspace for EBS CIO.', @userId, NULL, 9001, 0, @userId, 1, 
	1, 1, 0, NULL, 0, 0,
