...
namespace IdentitySwap
{
    ...
    public class IdentitySwappingForTesting
    {
        ...
        private static bool SwapIdentity(string keyForADPath)
        {
            bool result = false;
            ...
            try
            {
                ...
                if (impersonationAllowed)
                {
                    ...
                    HttpCookie cookieUser = HttpContext.Current.Request.Cookies[COOKIE_FOR_IMPERSONATION_USER];
                    HttpCookie cookieRoles = HttpContext.Current.Request.Cookies[COOKIE_FOR_IMPERSONATION_ROLES];
                    ...
                    if (cookieUser != null)
                    {
                        string userWithOrWithoutDomain = cookieUser.Values[VALUE];
                        ...
                        {
                            cookieUser.Values[VALUE] = userWithDomain;
                        }

                        // Extend the expiration time for the cookie
                        cookieUser.Expires = DateTime.Now.AddMinutes(CookieExpirationTimeInMinutes);

                        // Update the actual cookie
                        HttpContext.Current.Response.Cookies.Remove(COOKIE_FOR_IMPERSONATION_USER);
                        HttpContext.Current.Response.Cookies.Add(cookieUser);
                    }

                    if (cookieRoles != null)
                    {
                        roles = cookieRoles.Values[VALUE];
                        // Extend the expiration time for the cookie
                        cookieRoles.Expires = DateTime.Now.AddMinutes(CookieExpirationTimeInMinutes);

                        // Update the actual cookie
                        HttpContext.Current.Response.Cookies.Remove(COOKIE_FOR_IMPERSONATION_ROLES);
                        HttpContext.Current.Response.Cookies.Add(cookieRoles);
                    }
                    ...
                    if (!string.IsNullOrEmpty(userWithDomain))
                    {
                        ...
                        HttpContext.Current.User = impersonatedUser;
        ...
        public static void CreateCookieForSwappingIdentities(string userWithDomain, string roles = null)
        {
            HttpCookie cookie = HttpContext.Current.Request.Cookies[COOKIE_FOR_IMPERSONATION_USER] ?? new HttpCookie(COOKIE_FOR_IMPERSONATION_USER);
            cookie.Values[VALUE] = userWithDomain;
            cookie.Expires = DateTime.Now.AddMinutes(CookieExpirationTimeInMinutes);
            HttpContext.Current.Response.Cookies.Add(cookie);

            cookie = HttpContext.Current.Request.Cookies[COOKIE_FOR_IMPERSONATION_ROLES] ?? new HttpCookie(COOKIE_FOR_IMPERSONATION_ROLES);
            cookie.Values[VALUE] = roles;
            cookie.Expires = DateTime.Now.AddMinutes(CookieExpirationTimeInMinutes);
            HttpContext.Current.Response.Cookies.Add(cookie);
        }
        ...
        /// </summary>
        public static void ClearIdentitySwappingCookie()
        {
            HttpCookie cookieUser = HttpContext.Current.Request.Cookies[COOKIE_FOR_IMPERSONATION_USER];
            HttpCookie cookieRoles = HttpContext.Current.Request.Cookies[COOKIE_FOR_IMPERSONATION_ROLES];
            ...
            if (cookieUser != null)
            {
                cookieUser.Values[VALUE] = string.Empty;
                cookieUser.Expires = DateTime.Now.AddDays(-1);
            }

            if (cookieRoles != null)
            {
                cookieRoles.Values[VALUE] = string.Empty;
                cookieRoles.Expires = DateTime.Now.AddDays(-1);
            }
        }
        ...
        /// <returns>Boolean indicating whether impersonation was successful</returns>
        public static bool SwapIdentity(string keyForWhetherWeShouldProceed, string keyForADPath, string keyForAllowedGroups)
        {
            bool result = false;
            try
            {
                ...
                {
                    result = SwapIdentity(keyForADPath);
                }
            }
            catch
            {
