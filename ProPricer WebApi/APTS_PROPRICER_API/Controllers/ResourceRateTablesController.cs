...
namespace APTSPropricerApi.Controllers
{
    ...
    public class ResourceRateTablesController : ProPricerController
    {
        ...
        {
            using (IProPricerConnection ppc = (Connection.IProPricerConnection)PoolManager.GetInstance(instanceId).GetObjectsFromPool())
            {
                ...
                if (ppc.Workspace != null)
                {
                    try
                    {
                        ...
                        foreach (DirectRateTable rattbl in dircol)
                        {
